...
import { Icustom, IHoldItems, IOrderMiniDetails, IPackItem, IproductInfo, IQuantity, IreadyGroupTax, ITax } from "../../models/common-model";
...
export const OrdersComponent = (probs: any) => {
  ...
  const { shopId } = probs
  const { products, customers, taxes, taxGroups, variations, packageItems } = useContext(ProductContext);
  const { locationSettings, tailoringSizes, invoicDetails, tailoringExtras } = useContext(UserContext);
  ...
  const [selectedProductForVariation, setSelectedProductForVariation] = useState<{ product?: object, product_id: number, product_name: string, is_service: number, tailoring_id?: number, value?: string, tailoring_cart_index?: number, tailoringCustom?: Icustom }>({ product_id: 0, product_name: '', is_service: 0, value: "" });
  const [selectedHold, setSelectedHold] = useState<{ holdId: number }>({ holdId: -1 });
  const [isOrderEdit, setIsOrderEdit] = useState<number>(0);
  const [orderEditDetails, setOrderEditDetails] = useState<IOrderMiniDetails>({ isEdit: false, name: '', total_price: 0, orderId: 0 });
  ...
  const [defTaxGroup, setDefTaxGroup] = useState<IreadyGroupTax>({ primary: 0, nonPrimary: 0, excises: 0, serviesFixed: 0, servicesPercentage: 0 });
  const selectStyle = { control: (style: any) => ({ ...style, fontSize: '12px', border: "1px solid #efefef", borderRadius: '12px' }) }
  ...
  async function getOrderForEdit(barCodeId: number | string) {
    ...
    if (result.success) {
      ...
      if (result.newdata.length > 0)
        setOrderEditDetails({ isEdit: true, name: result.newdata[0].contact_name, total_price: result.newdata[0].total_price, orderId: 0, notes: result.newdata[0].notes })

      result.newdata.map((itm: any) => {
        _orders.push({ isEdit: true, transaction_id: itm.id, items: {}, type: itm.type, total_qty: itm.qty, quantity2: 0, product_id: itm.product_id, variation_id: itm.variation_id, name: itm.name, price: itm.price, is_service: itm.is_service, is_fabric: itm.is_fabric, def_tax: true, category_tax: 0, brand_tax: 0, never_tax: 0, product_tax: 0, is_tailoring: itm.is_tailoring, })
        if (itm.tailoring_link_num > 0)
          hasTailFabs++;
        _quantity.push({ freezeQuantity: parseFloat(itm.qty), freezeTaxAmount: parseFloat(itm.tax_amount), freezeTailoringCutsom: JSON.parse(itm.tailoring_custom), quantity: parseFloat(itm.qty), productIndex: -1, itemIndex: 0, prices: [{ stock_id: itm.stock_id, qty: itm.qty, price: itm.price, cost: 0 }], lineTotalPrice: itm.price, taxAmount: 0, tailoring: itm.tailoring_txt, tailoringCutsom: JSON.parse(itm.tailoring_custom), selectionColor: itm.tailoring_link_num, });
      })
      setOrders(_orders)
      setQuantity(_quantity)
      setIsOrderEdit(Number(barCodeId))
      if (hasTailFabs > 1)
        setIsEnableLink(true)
    }

  }
  const perperdForPrint = () => {
    let _items: any = [], totalTax = 0;
    if (quantity.length == 0 || quantity.length != orders.length)
      return

    quantity.map((qt: any, i: number) => {
      // qt.taxAmount 
      let _temp: any = [], _subPrice: any = []
  ...
  useEffect(() => {
    ClearOrders();
    setTimeout(() => { setAllowWork(true); }, 600)
  }, [router.asPath])
          ...
          <table className="table">
            <thead>
              ...
              <tr className="net-amount">
                ...
                <td></td>
                <td>{(totalAmount - subTotal).toFixed(locationSettings.currency_decimal_places)}</td>
              </tr>
              <tr className="net-amount">
                <td></td>
                <td className='txt-bold'>{invoicDetails.txtTotal} {invoicDetails.isMultiLang && invoicDetails.txtTotal2}</td>
                <td></td>
                <td className='txt-bold'>{Number(totalAmount).toFixed(locationSettings.currency_decimal_places)}</td>
              </tr>
            </thead>
          </table>
  ...
  function checkProductQtyinPackagesItems(qty: number, _product_id: number, _total_qty: number): boolean {
    let isAllowed = true;
    orders.map((or, index: number) => {
      if (or.type == 'package') {
        var ids = packageItems.filter((pi: IPackItem) => pi.parent_id == or.product_id).map((tm: any) => { return tm.product_id; });
  ...
  function addToCard(fromHold: boolean, index1: number, index2: number) {
    if (index1 != -1) {
      ...
      let idx: number = 0;
      idx = orders.findIndex((el: any) => { return (el.isEdit && el.quantity2 < 0 && el.product_id == _product_id) || (!el.isEdit && el.product_id == _product_id) })
      let qty = 1;
      if (idx > -1) {
      ...
      if (idx == -1 || fromHold) {
        ...
        if (itm.qty == 0 && itm.sell_over_stock == 1) {
          ...
          let _itm2 = { freezeQuantity: 0, quantity: 1, productIndex: index1, itemIndex: index2, prices: [{ stock_id: itm.stock_id, qty: 1, price: itm.price, cost: itm.cost }], lineTotalPrice: itm.price, taxAmount: 0 }
          _quantity.push(_itm2);
          quantityPush.push(_itm2);
        }
        setQuantity(_quantity);
  ...
  function addVarToCard(index1: number, index2: number, _variation_id: number | string, product_name = null) {
    ...
    if (index1 != -1 && index2 != -1) {
      ...
      if (idx == -1) {
        ...
        const _ord = { ..._itm, name: product_name == null ? selectedProductForVariation.product_name + ' - ' + _itm.name : product_name + ' - ' + _itm.name }
        ordersPush.push(_ord)
        setOrders([...tmpOrders, _ord]);
        let _quantity = [...quantity];
        const _quu = { freezeQuantity: 0, quantity: 1, productIndex: index1, itemIndex: index2, prices: [{ stock_id: _itm.stock_id, qty: 1, price: _itm.stock_id > 0 ? _itm.price : _itm.variation_price, cost: _itm.stock_id > 0 ? _itm.cost : _itm.variation_cost }], lineTotalPrice: 0, taxAmount: 0 };
        _quantity.push(_quu);
  ...
  async function displayOrders() {
    ...
    for (let i = 0; i < _orders.length; i++) {
      ...
      products.products_multi.map((topPro: IproductInfo[], index: number) => {
        if (index2 == -1) {
          index1 = index
          index2 = topPro.findIndex((itm: IproductInfo) => { return itm.product_id == _orders[i].product_id })
        }
      })
      if (index1 != -1) {
        ...
        if (_itm.is_tailoring > 0 || _itm.type == "tailoring_package") {
          ...
          quantityPush.push({
            freezeQuantity: 0,
            quantity: 1,
            productIndex: index1, itemIndex: index2,
  ...
  useEffect(() => {
    if (jobType.req == 1) {
      ...
      products.products_multi.map((topPro: IproductInfo[], index: number) => {
        if (index2 == -1) {
          index1 = index
          index2 = topPro.findIndex((itm: IproductInfo) => { return itm.product_id == product.product_id })
        }
      })
      if (index1 == -1)
        return
      ...
      if (jobType.custom?.isPackage) {
        ...
        products.products_multi.forEach((topPro: IproductInfo[], index: number) => {
          if (f_index2 == -1) {
            f_index1 = index
            f_index2 = topPro.findIndex((itm: IproductInfo) => { return itm.product_id == f_id })
          }
        });
        if (f_index1 == -1)
          return
      ...
      if (jobType.val2 != undefined && parseInt(jobType.val2!) != -1) {
        ...
        _quantity[parseInt(jobType.val2)].prices = [{ stock_id: 0, qty: _quantity[parseInt(jobType.val2)].quantity, price: _price, cost: product.cost }]
  ...
  useEffect(() => { ClearOrders(); }, [clearEvent]);
  //Clear On Refresh
  useEffect(() => {
    ClearOrders();
    setCustomer({ value: '1', label: 'walk-in customer', isNew: false })
  ...
  useEffect(() => {
    ...
    taxes.map((tx: ITax) => {
      if (tx.taxType == 'primary' && tx.isPrimary)
        _primary += tx.amount;
      else if (tx.taxType == 'primary')
        _none += tx.amount;
      else if (tx.taxType == 'excise')
        _excises += tx.amount;
    ...
    setDefTaxGroup({ primary: _primary / 100, nonPrimary: _none / 100, excises: _excises / 100, serviesFixed: _servies_fixed, servicesPercentage: _servies_percetage / 100 })
  ...
  function checkPackageItemsHasStock(orderQty: number) {
    var ids = packageItems.filter((pi: IPackItem) => pi.parent_id == product.product_id).map((itm: any) => { return itm.product_id; });
    var packageProducts: IproductInfo[] = products.products.filter((pro: IproductInfo) => ids.includes(pro.product_id))
    let hasStock = true;
    packageProducts.map(pp => {
  ...
  useEffect(() => {
    ...
    if (product.type == 'single' && product.is_tailoring > 0) {
      ...
      setSelectedProductForVariation({ product_id: product.product_id, product_name: product.name, is_service: product.is_service, tailoring_id: product.is_tailoring, value: "", tailoring_cart_index: 0 })
      setIsOpenTailoringDialog(true)
      return
    } else if (product.type == 'variable') {
      setSelectedProductForVariation({ product_id: product.product_id, product_name: product.name, is_service: product.is_service })
      setIsOpenVariationDialog(true)
      return
    } else if (product.is_service == 0 && product.type != 'package' && product.type != 'tailoring_package' && parseFloat(product.total_qty) == 0 && product.sell_over_stock == 0) {
      Toastify('error', 'Out Of Stock')
      return;
    }
    ...
    if (product.type == 'package' && !checkPackageItemsHasStock(1))
      return
    if (product.type == 'tailoring_package') {
      //tailoring_package
      setSelectedProductForVariation({ product: products.products_multi[index1][index2], product_id: product.product_id, product_name: product.name, is_service: product.is_service, tailoring_id: product.is_tailoring, value: "", tailoring_cart_index: 0 })
      setIsOpenTailoringDialog(true)
      return
    }
  ...
  const calculateTotol = () => {
    setProducInfo({ product_id: false });
    if (orders.length != quantity.length)
      return
    let _quantity = [...quantity];
    let totalWithoutTax: number = 0, _taxAmount = 0;
    ...
    orders.map((pro, idx) => {
      ...
      if (pro.isEdit && quantity[idx].freezeQuantity <= 0)
        return null
      if (pro.is_fabric == 1 || pro.is_tailoring! > 0)
        hasTailFabs++;
      if (pro.isEdit) {
        ...
        quantity[idx].prices.map((pr) => subItemTotal += (pr.price * pr.qty))
    ...
    if (type === 'plus') {
      ...
      if (_pro.type == 'package') {
        ...
        if (isVar)
          ...
          thePrices = JSON.parse(JSON.stringify(products.products_multi[_quantity[index].productIndex]))
        ...
        for (let i = 0; i < thePrices.length && qtyToAllocate > 0; i++) {
          ...
          if (stockItem.qty >= qtyToAllocate) {
            ...
            if (_quantity[index].prices.length > i)
              ...
              _quantity[index].prices.push({ stock_id: stockItem.stock_id, qty: stockItem.qty, price: stockItem.price, cost: stockItem.cost })
        ...
        if (qtyToAllocate > 0.5 && _orders[index].sell_over_stock!) {
          if (_quantity[index].prices[_quantity[index].prices.length - 1].stock_id != 0) {
            ...
            _quantity[index].prices[_quantity[index].prices.length - 1].qty = qt;
  ...
  const handleEditTailoring = (idx: number) => {
    if (quantity[idx].productIndex == -1) {
      ...
      products.products_multi.map((topPro: IproductInfo[], index: number) => {
        if (index2 == -1) {
          index1 = index
          index2 = topPro.findIndex((itm: IproductInfo) => { return itm.product_id == orders[idx].product_id })
        }
      })
      if (index1 == -1) {
  ...
  const barcodeHandleChange = (e: string) => {
    ...
    timeoutId = setTimeout(() => {
      ...
      e = _id;
      if (e.length > 0) {
        const mfil: any = products.products.filter((p: any) => p.name.toLowerCase().includes(e) || p.sku.toLowerCase().includes(e));
        if (mfil.length == 1) {
          SetProductsItems([])
          setProducInfo({ product_id: false });
          clearTimeout(timeoutId);
  ...
  return (
    ...
    <div className="card" style={{ width: '40%', marginLeft: '80px' }}>
      ...
      {isOpenVariationDialog && <VariationModal selectedProductForVariation={selectedProductForVariation} isOpenVariationDialog={isOpenVariationDialog} setIsOpenVariationDialog={setIsOpenVariationDialog} variations={variations.variations} />}
      {isOpenTailoringDialog && <TailoringModal shopId={shopId} selectedProduct={selectedProductForVariation} isOpenTailoringDialog={isOpenTailoringDialog} setIsOpenTailoringDialog={setIsOpenTailoringDialog} tailoringsData={tailoringSizes} tailoringExtras={tailoringExtras} />}
      <Customermodal shopId={shopId} showType={showType} userdata={customer} customers={customers} statusDialog={customerIsModal} openDialog={customerModalHandler} />
      <div className="card-body">
          ...
          <div className="d-flex">
            <div className="flex-grow-1">
              <Select
                ...
                options={customers}
                onChange={(choice: any) => setCustomer({ ...choice, isNew: choice.__isNew__ == undefined ? false : true })}
            ...
            <button
              ...
              onClick={() => {
                ...
                if (customer) {
                  ...
                  Toastify('error', "Choose Customer First!")
              }}
            >
          ...
          <div className="mt-2 p-2">
            <div className="search-with-icon search-pos">
              ...
              {productsItems.length > 0 && (
                <ul id="ui-id-1" className="container-popup-menu" style={{ position: 'absolute', top: '44px', left: '0px', width: '100%' }}>
                  {
                        ...
                        <li key={i} className="ui-menu-item">
                          <div onClick={
                            () => {
                              ...
                              setSearchProduct('')
                              setProducInfo({ product_id: false });
                              setTimeout(() => { setProducInfo(pro); }, 100)
                            }} >{pro.name} <span className="qty-barcode">( QTY: {pro.total_qty} ) - {pro.sku}</span></div>
                        </li>)
                    })
                  }
        ...
        <div
          ...
          {isOrderEdit > 0 && (<div className="edited-Order">
            <div >
              #{isOrderEdit}
            </div>
            <div >
          ...
          <table className="table m-font-size table-borderless text-center  align-middle mb-0">
            ...
            <tbody>
              ...
              {orders.length == quantity.length && orders.map((order, i) => {
                return (
                  <tr key={i} style={{ background: order.isEdit ? quantity[i].quantity == 0 ? '#ebbfbf' : '#bbe7e7' : '' }} >
                    {isEnableLink && <th scope="col">
                      <div className={isLinking ? 'non-cliclable-color cliclable-color' : 'non-cliclable-color'} style={{ background: colors[quantity[i].selectionColor!] }} onClick={() => handleColor(i)}></div>
                    </th>}
                    <th scope="row"> {i + 1} </th>
                    ...
                    <td>
                      <div className="input-step">
                        ...
                        />
                        <button type="button"
                          onClick={() => { quantityChange(i, 'plus'); }}
                          className="plus">
                          +
                        </button>
                      </div>
                      <span style={{ position: 'absolute', color: order.quantity2! > 0 ? 'green' : order.quantity2! < 0 ? 'red' : 'black', marginLeft: '4px' }} >{order.quantity2! > 0 ? '+' : ''}{order.quantity2! != 0 ? order.quantity2 : ''}</span>
                    </td>
                    <td className="text-end">
                      <span>{Number(quantity[i].lineTotalPrice).toFixed(locationSettings.currency_decimal_places)}</span>
                    </td>
                  </tr>
                );
              })}
        ...
        <OrderCalcs orderEditDetails={orderEditDetails} taxRate={taxRate} subTotal={subTotal} totalAmount={totalAmount} shippingRate={shippingRate} />

        <OrdersFooter selectedHold={selectedHold} orderEditDetails={orderEditDetails} shopId={shopId} details={{ taxRate, customerId: customer?.value, totalAmount, isReturn: isOrderEdit }} holdObj={{ orders, quantity, name: 'noset' }} />
      </div>
      {/* end card body */}
    </div >
  );
