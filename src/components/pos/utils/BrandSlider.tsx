...
const BrandSwiper = (props: any, swiper: ItypeSelected) => {
  ...
  useEffect(() => {
    ...
    if (selectedTab == 'brand')
      setItems(dataItems.brands)
    else if (selectedTab == 'category')
      setItems(dataItems.cats)
    ...
    <div className="swiper responsive-swiper rounded mb-3">
      {Loading ? "loadings.." :
        <Swiper
          ...
          className="mySwiper"
        >
          {items == null ? 'noo' : items?.map(({ name, id }, i) => {
            return (
              <SwiperSlide key={i}>
                <div
                  className={"swiper-slide swiper-slide-active" && (selectedTab == 'category' ? (activeCatId == id ? 'selectedCatClass' : '') : (activeBrdId == id ? 'selectedCatClass' : ''))}
                  onClick={() => {
