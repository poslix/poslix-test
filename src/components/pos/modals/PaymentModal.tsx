...
const PaymentModal = (probs: any) => {
    ...
    const { openDialog, statusDialog, holdObj, details, shopId, orderEditDetails, selectedHold } = probs;
    const [totalPaid, setTotalPaid] = useState<number>(0);
    const [mustPay, setMustPay] = useState<number>(0);
    const [difference, setDifference] = useState<number>(0);
    const [hasError, setHasError] = useState<{ st: boolean, msg: string }>({ st: false, msg: '' });
    const [, setJobType] = useRecoilState(cartJobType);
    const [paymentRows, setPaymentRows] = useState<IpaymentRow[]>([{ amount: 0, method: "cash", notes: "" }])
    const [canPay, setCanPay] = useState<boolean>(true)
    const [orderNote, setOrderNote] = useState<string>('')
    const selectStyle = { control: (style: any) => ({ ...style, fontSize: '12px', minWidth: '150px', maxHeight: '10px' }), menu: (base: any) => ({ ...base, fontSize: '12px' }), }
    ...
    useEffect(() => {
        ...
        let _mustPay = +Math.abs(details.totalAmount - orderEditDetails.total_price).toFixed(locationSettings.currency_decimal_places);
        setMustPay(_mustPay)
        ...
        holdObj.orders.map((od: any, i: number) => {
            if (od.type == "tailoring_package") {
                ...
                if (index !== -1)
                    fabs[index].qty += holdObj.quantity[i].tailoringCutsom?.fabric_length! * holdObj.quantity[i].quantity;
                else
                    fabs.push({ product_id: _id, qty: holdObj.quantity[i].tailoringCutsom?.fabric_length! * holdObj.quantity[i].quantity });
            }
        })
        holdObj.orders.map((od: any, i: number) => {
    ...
    const newPaymentRow = (): void => {
        ...
        rows.push({ amount: difference < 0 ? Math.abs(difference) : 0, method: "cash", notes: "" });
        setPaymentRows(rows);
        calculation(rows)
    }
    useEffect(() => {
        if (orderEditDetails.isEdit) {
            setCanPay(totalPaid >= mustPay)
            setDifference(+Number(totalPaid - mustPay).toFixed(locationSettings.currency_decimal_places))
        } else {
            setCanPay(totalPaid.toFixed(locationSettings.currency_decimal_places) >= details.totalAmount.toFixed(locationSettings.currency_decimal_places))
            setDifference(+Number(totalPaid - details.totalAmount).toFixed(locationSettings.currency_decimal_places))
        }
    }, [totalPaid])
    const handlePayment = () => {
        let isOk = true;
            ...
            if (holdObj.orders[index].is_service == 0) {
                if (holdObj.orders[index].variation_id > 0) {
                    ...
                    if (qt.productIndex != -1) {
                        ...
                        _variations[qt.productIndex] = { ..._variations[qt.productIndex], total_qty: _variations[qt.productIndex].total_qty - sumQty };
                    }
                } else {
                    //for products
                    if (qt.productIndex != -1) {
                        ...
                        const _dd = { ..._products[qt.productIndex], total_qty: _products[qt.productIndex].total_qty - sumQty };
    ...
    async function insertPayment() {
        ...
        var result = await apiInsertCtr({ type: 'transactions', subType: 'newPosSale', data: { items: holdObj, details, paymentRows, orderEditDetails, orderNote }, shopId })
        if (result.success) {
            Toastify('success', 'successfully done')
            openDialog(false);
            setJobType({ req: 2, val: orderNote, val2: result.newdata })
            if (holdItems.length > 0 && selectedHold != undefined && selectedHold.holdId != null && selectedHold.holdId > -1) {
    ...
    function fixAmount() {
        if (paymentRows.length > 0) {
            const _rows: any = [...paymentRows];
            if (paymentRows.length == 1)
                paymentRows[0].amount = mustPay;
            else
                difference > 0 ? paymentRows[paymentRows.length - 1].amount! -= difference : paymentRows[paymentRows.length - 1].amount! += difference;
            setPaymentRows(_rows);
            calculation(_rows);
        }
    }
        ...
        <>
            ...
            <Dialog
                ...
                <DialogContent className="poslix-modal-content">
                    <div className="modal-body">
                        <div>
                            <div className="payment-item">
                                <label className="label" htmlFor="abount">Order Note</label>
                                <textarea className="form-control" name='order_note' maxLength={80} onChange={(e) => setOrderNote(e.target.value)} value={orderNote} />
                            </div>
                            {
                                paymentRows.map((paymentRow, i: number) => {
                                    return (
                                        <div key={i} className='payment-box'>
                                            <div className="payment-item">
                                                <label className="label" htmlFor="abount">Amount</label>
                                                <input className="form-control" type='number' name="amount" onChange={(evnt) => (paymentRowChange(i, evnt))} value={paymentRow.amount} />
                                            </div>

                                            <div className="payment-item">
                                                <label className="label" htmlFor="abount">Method </label>
                                                <Select styles={selectStyle} options={paymentMethods} onChange={(evnt: any) => (paymentRowChange(i, evnt))}
                                                    value={paymentMethods.find(f => { return f.value == paymentRow.method })}
                                                />
                                            </div>

                                            <div className="payment-item">
                                                <label className="label" htmlFor="abount">Pay Note</label>
                                                <input className="form-control" type='text' name='notes' onChange={(evnt) => (paymentRowChange(i, evnt))} value={paymentRow.notes} />
                                            </div>
                                            <div className="payment-item">
                                                <label className="label" htmlFor="abount">Action</label>
                                                <ButtonGroup className="mb-2 m-buttons-style"><Button onClick={() => paymentRowDel(i)}><FontAwesomeIcon icon={faTrash} /></Button></ButtonGroup>
                                            </div>
                                        </div>
                                    )
                                })
                    ...
                    {difference != 0 ? (<div className='payment-rows-details' onClick={fixAmount}>{difference > 0 ? 'Return : ' + difference : 'Add More : ' + difference}</div>) : null}
                    <div className="modal-footer">
                        ...
                        <button
                            type="button"
                            className={"btn btn-label " + (canPay ? "btn-primary" : "btn-danger") + " right nexttab"}
                            data-nexttab="pills-finish-tab"
