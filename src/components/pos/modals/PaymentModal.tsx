...
const PaymentModal = (probs: any) => {
  ...
  const { tax, __WithDiscountFeature__total, setDiscount, totalDiscount } =
    probs;
    console.log('hiiiiiiiiiiiiiiiiii',__WithDiscountFeature__total);
  ...
  useEffect(() => {
    ...
    console.log(totalPaid , details.totalAmount , details.subTotal);
  ...
  const paymentRowChange = (index: any, evnt: any): void => {
    ...
    // mohamed elsayed
      ...
      if(name === 'amount'){
        ...
        console.log('return', _rows[index]['return']);
        _rows[index]['totalDue'] = 0
        
        }else{
          _rows[index]['totalDue'] =   Number(diff)
        console.log('totalDue', _rows[index]['totalDue']);
  ...
  const newPaymentRow = (): void => {
    ...
    setToBeAdded(totals);
    console.log('مجموع الاتنين', Number(rows[rows.length-1].amount) + Number((Math.abs(difference))));
  ...
  async function insertPayment() {
    ...
    if (result.success) {
      console.log('reeeeeeessssssssult',result);
                ...
                <div key={i} className="payment-box">
                  <div className="view_payment" >
                    <div className="view_payment_item">
                      ...
                      <span>{locationSettings.currency_code+" "+paymentRow.total}</span>
                    </div>
                    <div className="view_payment_item">
                      <span>Total paying</span>
                      <span> {locationSettings.currency_code+" "+(Number(paymentRow.amount) + toBeAdded).toString()}</span>
                    </div>
                    <div className="view_payment_item">
                      <span>Change Return</span>
                      <span> {locationSettings.currency_code+" "+paymentRow.return }</span>
                    </div>
                    <div className="view_payment_item">
                      <span>balance</span>
                      <span>{locationSettings.currency_code+" "+paymentRow.totalDue }</span>
                    </div>
                  </div>
                </div>
              );
