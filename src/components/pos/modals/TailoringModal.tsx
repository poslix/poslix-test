...
const TailoringModal = (props: any) => {
  ...
  async function initDataPage(typeId = '0') {
    const { success, newdata } = await apiFetchCtr({
      fetch: 'pos',
      subType: 'initTailoringUserSizes',
      shopId,
      typeId,
    });
    if (!success) {
  ...
  useEffect(() => {
    let id =
      selectedProduct.product != undefined && selectedProduct.product.type == 'tailoring_package'
        ? selectedProduct.product.tailoring_type_id
        : selectedProduct.tailoring_id;
    initDataPage(id);
  }, []);
  useEffect(() => {
    if (
      selectedProduct.product != undefined &&
      selectedProduct.product.type == 'tailoring_package'
    ) {
      ...
      if (selectedProduct.tailoringCustom != undefined) {
        ...
        setSelectedExtras(
          selectedProduct.tailoringCustom.extras != undefined &&
            selectedProduct.tailoringCustom.extras.length > 3
            ? JSON.parse(selectedProduct.tailoringCustom.extras)
            : []
        );
      }
      ...
      if (selectedProduct.value.length > 4) {
        ...
        const _sizes = JSON.parse(
          JSON.stringify(tailoringsData[selectedProduct.product.tailoring_type_id])
        );
      ...
      if (selectedFab == -1) setSelectedFab(Array(filtered_products.length).fill(0));
      else {
      ...
      //for extras product
      if (
        selectedProduct.product.product_ids != null &&
        selectedProduct.product.product_ids.length > 1
      ) {
      ...
      if (selectedProduct.value.length > 4) {
        ...
        var _items = [
          ...newItems,
          { id: 0, tailoring_type_id: 0, name: '_frm_name', is_primary: 0 },
        ];
        setSelectedSizes([_items]);
      }
    }
  ...
  const handleAddMore = () => {
    ...
    newItems.map((d: any) => (d['value'] = ''));
    if (selectedProduct.value.length > 4) _data.push([...newItems]);
    else
  ...
  const handleFabricSelect = (i: number) => {
    ...
    for (let j = 0; j < _data.length; j++) _data[j] = 0;
    _data[i] = 1;
  ...
  const handleDefValue = (traId: number) => {
    ...
    if (ind > -1)
      return {
        value: selectedExtras[ind].value,
        label: selectedExtras[ind].value,
        id: selectedExtras[ind].id,
      };
    else return { value: '', label: '', id: -1 };
  };
  return (
    <>
      <Dialog open={isOpenTailoringDialog} className="poslix-modal">
        <DialogTitle className="poslix-modal-title text-primary">Complete The Form</DialogTitle>
          ...
          {!isLoading ? (
            <div className="poslix-modal">
              <div className="modal-content">
                    ...
                    return (
                      <div key={i + 99} className="container-tailoring">
                        {Isizes.map((se: any, ix: number) => {
                          return (
                            <div key={i + ix + 1} className="tailor-item">
                              {' '}
                              <input
                                type={se.name != '_frm_name' ? 'number' : 'text'}
                                min={0}
                                className={se.is_primary == 1 ? 'iskey' : ''}
                                placeholder={se.name == '_frm_name' ? 'Size Name' : se.name}
                                value={se.value}
                                name={'valname' + (i + ix)}
                                onChange={(e) => handleChange(e, i, ix)}
                              />
                            </div>
                          );
                        })}
                    ...
                    <div className="packitems-container">
                      {fabrics.map((fab: any, i: number) => {
                        return (
                          <div
                            key={i}
                            className={`packitems ${selectedFab[i] == 1 ? 'active' : ''}`}
                            onClick={() => handleFabricSelect(i)}>
                            <img src={fab.image} />
                            <h5>{fab.name}</h5>
                ...
                <div className="packitems-container">
                  <textarea
                    className="tailoring-notes"
                    value={txtNote}
                    onChange={(e) => setTxtNote(e.target.value)}></textarea>
                </div>
                {isPackage && (
                  <h5 className="status-price">
                    Line Total:{' '}
                ...
                <div className="modal-footer">
                  ...
                  <button type="button" className="popup-btn btn btn-primary" onClick={handleClick}>
                    Save
                  </button>
                </div>
              </div>
