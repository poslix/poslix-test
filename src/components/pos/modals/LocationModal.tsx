...
const LocationModal = ({showDialog, setShowDialog, locations, data, setData, shopId, value, refetch}) => {
    ...
    const onSubmit = async () => {
        const { success, newdata } = await apiUpdateCtr({ type: 'products', subType: 'changeItemsLocation', shopId,
        ...
        Toastify('success', 'Items location has changed successfully...')
        refetch()
        setShowDialog(false)
        setData([])
        setSelectedShop(locations[value])
