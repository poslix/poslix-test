...
const AddNewUser = (probs: any) => {
    ...
    const { index } = probs;
    const [formObj, setFormObj] = useState<userDashboard>({ isNew: true, name: '', username: '', password: '', mobile: '', email: '' })
    const [errorForm, setErrorForm] = useState({ name: false, email: false, email2: false, email3: false, password: false })
    async function insertUpdateUsers() {
        const { success, data, msg, code } = await apiInsertCtr({ type: 'owner', subType: 'addUpdatebusinessUsers', data: formObj })
        console.log(success);
        if (!success) {
            Toastify('error', msg);
            if (code == 100)
        ...
        if (formObj.isNew)
            probs.users.push({ ...formObj, id: data });
    ...
    useEffect(() => {
        if (index != undefined && index > 0) {
            ...
            if (_idn > -1)
                setFormObj({ ...probs.users[_idn], isNew: false })
            else {
                                ...
                                <div className="col-md-12">
                                    ...
                                    <div className="col-md-6">
                                        <div className="form-group2">
                                            ...
                                            {!errorForm.email && errorForm.email2 && <p className='p-1 h6 text-danger '>Please enter a valid email address</p>}
                                            {errorForm.email3 && <p className='p-1 h6 text-danger '>This Email Already Exist,Use Another One</p>}
                                        </div>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="form-group2">
                                            <label>Password: <span className='text-danger'>*</span></label>
                                            <input type="text" className="form-control" placeholder="" value={formObj.password}
