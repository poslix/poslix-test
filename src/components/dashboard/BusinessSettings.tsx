...
const BusinessSettings = ({ username, businessId }: any) => {
  ...
  const [locations, setLocations] = useState<
    { id: number; name: string; currency_id: number; decimal_places: number }[]
  >([]);
  ...
  const [businessUsers, setBusinessUsers] = useState<
    { value: number; label: string; level: string; stuff_ids: string; locationId: number }[]
  >([]);
  ...
  async function addEditUserStuf() {
    var result = await apiInsertCtr({
      type: 'owner',
      subType: 'addupdateUserStuff',
      isEdit: isEditedStuff,
      businessId,
      locationId: shopId,
      data: { stuffs: selectedRoles, user: selectedUserId },
    });
    const { success, newdata } = result;
    if (success) {
      const _idx = businessUsers.findIndex((el: any) => {
        return el.value == selectedUserId && el.locationId == shopId;
      });
      var _data: any = businessUsers;
      if (_idx > -1) _data[_idx].stuff_ids = newdata;
      else _data.push(newdata);
  ...
  async function updateSettings(settingType: string, index = 0) {
    var result = await apiUpdateCtr({
      ...
      subType: settingType,
      genSettings: formObj,
      locationSetting: locations[index],
      data: { businessId, locationId: shopId },
    });
    ...
    if (success) {
      ...
      const currency_code = currencies.filter((c: any) => {
        return c.value == locations[index].currency_id;
      })[0].name;
      ...
      const modifiedArray = myArray.map((obj) => {
        if (obj.value === objectId) {
          ...
          return {
            ...
            currency_code: currency_code.substring(
              currency_code.lastIndexOf('(') + 1,
              currency_code.lastIndexOf(')')
            ),
    ...
    <div className="row">
      <div className="col-md-12">
        <button
          ...
          <FontAwesomeIcon icon={faArrowAltCircleLeft} /> My Business List{' '}
        </button>
        <Tabs
                      ...
                      <div className="row">
                        <div className="col-md-4 col-lg-4 col-cm-4">
                          ...
                          <div className="form-group2">
                            ...
                            />
                            {errorForm.name && (
                              <p className="p-1 h6 text-danger ">Enter Business name</p>
                            )}
                          </div>
                          <div className="form-group2">
                            <label>
            ...
            return (
              <Tab key={i} eventKey={loc.id} title={loc.name}>
                <Card>
                  ...
                  <Card.Body className="">
                    {!showAddUser && (
                      <div className="setting-locations">
                        <div className="col-md-4 col-lg-4 col-cm-4">
                          <div className="form-group">
                            ...
                            <input
                              ...
                              value={loc.name}
                              onChange={(e) => {
                                handelChangeLocationSettings(e, i);
                              }}
                            />
                          </div>
                          <div className="form-group">
                            ...
                            <Select
                              ...
                              value={currencies.filter((f: any) => {
                                return f.value == loc.currency_id;
                              })}
                              onChange={(e) => {
                                handelChangeLocationSettings(e, i);
                              }}
                          ...
                          <div className="form-group">
                            <label>
                              Decimal Place: <span className="text-danger">*</span>
                            </label>
                            <input
                              type="number"
                              name="decimal_places"
                                ...
                                return (
                                  <tr key={i}>
                                    ...
                                    <td>
                                      {showPropryRoles(
                                        businessUsers.find(
                                          (ee) => ee.value == bs.value && ee.locationId == shopId
                                        )?.stuff_ids || 'no'
                                      )}
                                    </td>
                                    <td>
                                          ...
                                          onClick={() => {
                                            ...
                                            var isNew =
                                              businessUsers.findIndex(
                                                (ee) =>
                                                  ee.value == bs.value && ee.locationId == shopId
                                              ) > -1;
                                            var _stuf = (
                                              businessUsers.find(
                                                (ee) =>
                          ...
                          <div className="col-md-6 col-lg-6 col-cm-6">
                            ...
                            <div className="form-group">
                              ...
                              <Select
                                options={allusers}
                                isDisabled={true}
                                value={allusers.filter((f: any) => {
                                  return f.value == selectedUserId;
                                })}
                                onChange={(itm) => {
