...
const OrdersTable = (probs: any) => {
  ...
  const [tailoringExtras, setTailoringExtras] = useState<
  ...
  const handleClickShowDetials = (_ind: number) => {
    ...
    if (index != -1) {
      ...
      if (
        orders[index].tailoring_custom != null &&
        orders[index].tailoring_custom.length > 5
      ) {
        //its package
        ...
        setTailoringExtras(
          _packs.extras != undefined && _packs.extras.length > 3
            ? JSON.parse(_packs.extras)
            : []
        );
                  ...
                  {selectedSizes.map((size, i) => {
                    return (
                      <div key={i + 1} className="order-details-item">
                        <span>
                          {size.name == "_frm_name" ? "name" : size.name}
                        </span>{" "}
                        <br /> {size.value}
                      </div>
                    );
                ...
                <div className="tailoring-notes">
                  ...
                  {tailoringExtras.map((te, i: number) => {
                    return (
                      <p key={i}>
                        <span>
                          {extras.find((ex: any) => ex.id == te.id)?.name}
                        </span>{" "}
                        {te.value}
                      </p>
                    );
