...
export default function DatePicker({
  ...
  useEffect(() => {
    if (!selectedRange) {
      ...
      const selectedVal = options.find(
        (option) => option.range[0] === selectedRange[0]
      );
  ...
  return (
    ...
    <div className="w-fit rounded" style={{border: "0.5px solid #ccc"}}>
      <ReactDatePicker selected={startDatepicker} startDate={startDatepicker} endDate={endDatepicker}
        selectsRange dateFormat="yyyy/MM/dd"
