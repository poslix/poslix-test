      ...
      if (repo.status === true) {
        ...
        if (locationPermission(repo.data.locs, shopId) != -1) {
          if (subType == 'insetUpdateExpenes') {
            ...
            const insertedIds = [];
            data.map((itm: ITax) => {
              if (itm.isNew && itm.name.length > 1) sqlValues.push([shopId, itm.name]);
            });
          ...
          if (subType == 'addExpenseList') {
            ...
            var con = doConnect();
            con.query(
              `INSERT INTO expenses_list(location_id,name,amount,expense_id,attach,date,created_by,created_at) VALUES (?,?,?,?,?,?,?,?)`,
              [
                shopId,
                data.name,
                data.amount,
              ...
              function (err: QueryError, reg: any) {
                if (err) {
                  ...
                  res
                    .status(200)
                    .json({ success: true, msg: 'Expense Added', newdata: reg.insertId });
                  res.end();
                  con.end();
                  return;
                }
