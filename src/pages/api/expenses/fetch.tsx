            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == "getCateAndList") {
                        ...
                        //get all taxes
                        await con.promise()
                            .query(`SELECT expenses_list.*,expenses.name AS category
                                FROM expenses_list 
                                    INNER JOIN expenses ON expenses.id =expenses_list.expense_id
                                    WHERE expenses_list.location_id = ? ORDER BY expenses_list.id desc`, [shopId])
                        ...
                        con.query(`SELECT id,name FROM expenses
                                        WHERE location_id = ? ORDER BY id;`, [shopId], function (err: QueryError, cate: any) {
                            if (err) {
                                console.log(err);
