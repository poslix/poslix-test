            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == "getTaxs") {
                        ...
                        WHERE  location_id = ?;`, [shopId], function (err: QueryError, data: any) {
                        ...
                        con.query(`SELECT tax_rates.id as 'tax_id',tax_rates.name,tax_rates.amount,tax_rates.type,tax_rates.tax_type,tax_group.id
                                        FROM tax_rates
                                            INNER JOIN tax_group ON tax_group.tax_id = tax_rates.id
                                                WHERE tax_rates.location_id = ? AND tax_group.parent_id = ? ORDER BY tax_rates.id DESC;`, [shopId, id], function (err: QueryError, data: any) {
                            if (err) {
                                res.setHeader('Content-Type', 'application/json');
