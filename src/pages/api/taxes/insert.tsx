            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == "insetUpdatePrimaryTax") {
                        ...
                        const sqlCondi = 'INSERT INTO tax_rates(location_id,name,amount,created_by,type,is_primary,tax_type) VALUES ?'
                        const sqlValues: any = [];
                        data.map((itm: ITax, idx: number) => {
                            if (itm.isNew && itm.name.length > 1)
                                sqlValues.push([shopId, itm.name, Number(itm.amount), repo.data.id, itm.taxType != 'service' ? 'percentage' : itm.amountType, itm.isPrimary, itm.taxType])
                        });
                    ...
                    if (subType == 'insertGroupTax') {
                        ...
                        if (isDefault) {
                            await con.promise().query('UPDATE tax_rates SET is_primary = ? WHERE tax_type = ? and location_id = ?', [0, 'group', shopId])
                                .then((rows: any, fields: any) => { })
                                .catch()
                        }
                        if (id != 0) {
                            //its edit
                            await con.promise().query('UPDATE tax_rates SET is_primary = ?,name=? WHERE id = ?', [isDefault ? 1 : 0, gname, id])
                                .then((rows: any, fields: any) => { })
                                .catch()
                            const sqlValues: any = [];
                            ...
                            if (sqlValuesDel.length > 0) {
                                ...
                                await con.promise().query(sqlCondiDel, sqlValuesDel)
                                    .then((rows: any, fields: any) => { })
                                    .catch()
                            }
                            //insert new items
                            if (sqlValues.length > 0) {
                                ...
                                sqlValues.map(async (obj: any) => {
                                    console.log(obj.location_id, obj.parent_id, obj.tax_id);
                                    await con.promise().query(`INSERT INTO tax_group (location_id, parent_id, tax_id)
                                                    SELECT ?, ?, ?
                                                        WHERE NOT EXISTS (
                                                            SELECT 1 FROM tax_group WHERE location_id = ? AND parent_id = ? AND tax_id = ?);`,
                                        [obj.location_id, obj.parent_id, obj.tax_id, obj.location_id, obj.parent_id, obj.tax_id])
                                        .then((rows: any, fields: any) => {
                            ...
                            con.query(`INSERT INTO tax_rates(location_id,name,amount,created_by,type,is_primary,tax_type) VALUES (?,?,?,?,?,?,?)`,
                                ...
                                function (err: QueryError, prov: any) {
                                    ...
                                    data.map((itm: any, idx: number) => {
                                        if (itm.isChoosed)
                                            sqlValues.push([shopId, prov.insertId, itm.id])
                                    });
