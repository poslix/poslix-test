            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == 'getItemsReport') {
                        let sales: any = [], sums: any = [];
                        var con = doConnect();
                        //get last sales
                        await con.promise()
                            .query(`SELECT tl.id,tl.product_id,tl.tax_amount,tl.variation_id,CONCAT_WS(' ',s.name,pv.name) AS 'name',ts.notes,s.type,s.is_service,tl.stock_id,tl.qty,tl.cost,tl.price,ts.contact_id,ts.total_price,CONCAT_WS(' ',cont.first_name,cont.last_name) AS 'contact_name',s.is_fabric,tl.note,DATE_FORMAT(ts.created_at, '%Y-%m-%d %H:%i:%s') AS created_at
                            FROM transactions_lines tl
                               ...
                               WHERE ts.location_id = ? AND ts.type = "sell" and ts.created_at >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
                                          AND ts.created_at < CURDATE();`, [2, 100])
                        ...
                        await con.promise()
                            .query(`SELECT sum(tl.tax_amount) AS tax,sum(tl.cost) AS cost,sum(tl.price) AS price,sum(ts.total_price) AS subTotal
                            FROM transactions_lines tl
                                left JOIN products s ON s.id = tl.product_id
                                left JOIN product_variations pv ON (pv.id = tl.variation_id AND pv.parent_id = tl.product_id)
                                left JOIN transactions ts ON ts.id = tl.transaction_id
                        ...
                        await con.promise()
                            .query(`SELECT sum(transactions.total_price) AS subTotal,
                            sum(transactions.tax_amount) AS tax
                              FROM transactions
                                    WHERE transactions.location_id = ? 
                                          AND transactions.type = 'sell' 
                                              ORDER BY id DESC LIMIT 500 ;`, [shopId])
                            .then((rows: any, fields: any) => {
                                sums = rows[0]
                            })
                            .catch((err: QueryError) => { })
                        ...
                        var con = doConnect();
                        await con.promise()
                            .query(`SELECT cr.*,CONCAT_WS(' ',users.first_name,users.last_name) AS 'name'
                            FROM cash_registers cr
                                inner JOIN users ON users.id = cr.user_id
                                WHERE cr.location_id = ?  ORDER BY id desc`, [shopId])
                            .then((rows: any, fields: any) => {
                                users = rows[0]
                            })
                            .catch((err: QueryError) => { })
