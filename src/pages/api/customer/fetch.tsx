                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == "getCustomerInfo") {
                        ...
                        con.query(`SELECT id,first_name,last_name,city,state,country,address_line_1 AS 'addr1',address_line_2 AS 'addr2' ,mobile,country,zip_code,shipping_address
                            FROM contacts
                                WHERE id = ?`, [theId], function (err: QueryError, prods: RowDataPacket[]) {
                            if (err) {
                    ...
                    if (subType == "getCustomerlist") {
                        var con = doConnect();
                        con.query(`SELECT id,CONCAT_WS(' ',first_name,last_name) AS 'name',mobile
                            FROM contacts WHERE contacts.location_id = ? ORDER BY id desc`, [shopId], function (err: QueryError, prods: RowDataPacket[]) {
                            if (err) {
                                res.setHeader('Content-Type', 'application/json');
                                res.status(200).json({ success: false, msg: 'error', newdata: [] });
                                res.end();
