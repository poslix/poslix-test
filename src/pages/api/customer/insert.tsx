      ...
      if (repo.status === true) {
        ...
        if (locationPermission(repo.data.locs, shopId) != -1) {
          if (subType == 'addCustomer') {
            ...
            await con
              .promise()
              .query(`SELECT id FROM contacts WHERE mobile = ? AND location_id = ? LIMIT 1`, [
                data.mobile,
                shopId,
              ])
            ...
            con.query(
              `INSERT INTO contacts(location_id,type,first_name,last_name,mobile,city,state,country,address_line_1,address_line_2,zip_code,shipping_address,created_by,created_at) 
                            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
              [
                shopId,
                'customer',
          ...
          if (subType == 'opens') {
            ...
            var con = doConnect();
            con.query(
              `INSERT INTO cash_registers(location_id,user_id,STATUS,closing_amount,created_at) VALUES(?,?,?,?,?)`,
