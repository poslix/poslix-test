                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    if (subType == 'insertProducts') {
                        ...
                        for (let index = 0; index < req.body.data.length; index++) {
                            ...
                            await con.promise().query(`insert into products (name,type,location_id,image,sku,alert_quantity,sell_price)
                                VALUES (?,?,?,?,?,?,?)
                                `, [element.name,element.Type,element.location_id,element.image,element.sku,element.alertQuantity,element.sell_price])
                            .then()
                            .catch()
                        }
                        res.setHeader('Content-Type', 'application/json');
