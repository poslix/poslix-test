          ...
          if (subType == 'insertProducts') {
            ...
            if (_hasSku) {
              redirection(
                ...
                con,
                res,
                'The ' + fdata.sku + ' Already Exist,Use Another One',
                100
              );
              return;
            }
            con.query(
              ...
              [
                ...
                fdata.productName2,
                fdata.type,
                fdata.type == 'tailoring_package' ? 0 : fdata.isTailoring,
                shopId,
                img,
                fdata.tax_id,
                never_tax,
              ...
              async function (err: QueryError, prods: any) {
                if (err) {
                  ...
                  res
                    .status(200)
                    .json({ success: false, msg: 'somthing is Wrong,Error Code AP201' });
                  res.end();
                  con.end();
                  return;
                }
                ...
                if (fdata.type == 'package') {
                  ...
                  if (fdata.variations.length > 1) {
                    ...
                    if (valueAddVariation.length > 0) {
                      ...
                      await con
                        .promise()
                        .query(`DELETE from products WHERE id  = ? LIMIT 1`, [prods.insertId])
                        .then((rows: any, fields: any) => {})
                        .catch();
                  ...
                  if (_prices.length == 0) {
                    redirection(
                      ...
                      con,
                      res,
                      'Price Rules Are Not Enterd, Please Enter The Prices'
                    );
                    return;
                  }
                  if (_fabs.length == 0) {
