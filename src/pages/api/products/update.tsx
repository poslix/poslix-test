            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == 'editProduct') {
                        ...
                        let _hasSku = false;
                        var con = doConnect();
                        await con.promise().query(`SELECT sku FROM products WHERE sku=? AND location_id= ? LIMIT 1`, [data.sku, shopId])
                        ...
                        if (data.type == "package") {
                            ...
                            if (selectedProducts.length > 1) {
                                selectedProducts.map((sp: any, idx: number) => {
                                    if (sp.isNew)
                                        ValueAddPackageItems.push([shopId, data.id, sp.product_id, sp.price, repo.data.id])
                                });
                                ...
                                for (let idx = 0; idx < selectedProducts.length; idx++) {
                                    if (selectedProducts[idx].isNew == 0 && selectedProducts[idx].price > 0) {
                                        await con.promise().query('UPDATE package_items SET price = ? WHERE id = ?', [parseFloat(selectedProducts[idx].price), parseInt(selectedProducts[idx].pack_id)])
                                            .then((rows: any, fields: any) => {
                                            })
                                            .catch()
                                    }
                            ...
                            if (data.variations.length > 1) {
                                data.variations.map((sp: any, idx: number) => {
                                    if (sp.isNew && sp.name.length > 0)
                                        valueAddVariation.push([shopId, data.id, sp.name, sp.name2, sp.sku, sp.cost, sp.price, data.isSellOverStock, data.isMultiPrice, data.isService, repo.data.id])
                                });
                                //insert
                                ...
                                for (let idx = 0; idx < data.variations.length; idx++) {
                                    if (!data.variations[idx].isNew && data.variations[idx].price > 0) {
                                        await con.promise().query('UPDATE product_variations SET name = ?, name2 = ?,sku = ?,cost = ?,price = ?,sell_over_stock = ?,is_service = ? WHERE id = ?',
                                            [data.variations[idx].name, data.variations[idx].name2, data.variations[idx].sku, data.variations[idx].cost, data.variations[idx].price, data.isSellOverStock, data.isService, data.variations[idx].id])
                                            .then((rows: any, fields: any) => {
                                            })
                                            .catch()
                                    }
                            ...
                            const sqlAddVariation = 'UPDATE `tailoring_package` set tailoring_type_id = ?,prices_json = ?,fabric_ids = ? where location_id = ? and parent_id = ?';
                            await con.promise().query(sqlAddVariation, [data.isTailoring, JSON.stringify(_prices), _fabs, shopId, data.id])
                                .then((rows: any, fields: any) => {
                                    console.log('update package tailoring ', rows);
                                })
                        ...
                        con.query(`UPDATE products SET 
                            ...
                            WHERE id=?`,
                            [data.productName, data.productName2, data.type, img, data.tax_id, isNever, data.sku, data.barcodeType, data.unit, data.brand, data.cat, data.alertQuantity, data.price, data.cost,
                            data.isService, data.isFabric, data.isSellOverStock, data.isMultiPrice, data.isFifo, data.isTailoring, data.id],
                        ...
                        con.end()
                        return;
                    } else if (subType == 'changeItemsLocation') {
                        var con = doConnect();
                        await con.promise().query(`UPDATE products 
                            SET location_id = ?
                            WHERE id IN (?)`, [req.body.data.newShopId, req.body.data.items])
                            .then()
                            .catch()
                        res.setHeader('Content-Type', 'application/json');
                        res.json({
