        ...
        if (locationPermission(repo.data.locs, shopId) != -1) {
          ...
          if (subType == 'getList') {
            ...
            await con
              .promise()
              .query(
                `SELECT tt.id,tt.name AS 'type_name',tt.multiple_value,ts.is_primary,ts.name
                            FROM tailoring_type tt
                            INNER JOIN tailoring_sizes ts ON ts.tailoring_type_id =tt.id 
                            WHERE tt.location_id  = ? AND ts.is_primary = 1`,
                [shopId]
              )
            ...
            const _ext = await con.promise().query(
              `SELECT *
                            FROM tailoring_extra
                                WHERE tailoring_extra.location_id = ?`,
              [shopId]
            );
            extras = _ext[0];
            //sen headers
          ...
          if (subType == 'getInitPage') {
            ...
            await con
              .promise()
              .query(
                `SELECT name as 'label',id as value FROM tailoring_extra WHERE location_id  = ?`,
                [shopId]
              )
              .then((rows: any, fields: any) => {
                extras = rows[0];
            ...
            if (id != '0') {
              ...
              await con
                .promise()
                .query(
                  `SELECT id,name,is_primary as 'isPrimary' FROM tailoring_sizes WHERE tailoring_type_id  = ?`,
                  [id]
                )
                .then((rows: any, fields: any) => {
                  sizes = rows[0];
          ...
          if (subType == 'getInitExtra') {
            ...
            await con
              .promise()
              .query(`SELECT * FROM tailoring_extra WHERE location_id  = ? and id = ?`, [
                shopId,
                id,
              ])
              .then((rows: any, fields: any) => {
          ...
          if (subType == 'getOrders') {
            ...
            await con
              .promise()
              .query(
                `SELECT ts.id,ts.transaction_id,ts.product_id,ts.tailoring_txt,ts.tailoring_custom,ts.note,ts.status,s.created_at,tails.name as 'type_name',ts.tailoring_link_num,contacts.name AS 'contact_name',contacts.mobile AS 'contact_mobile'
