            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == "delTailoring") {
                        ...
                        AND NOT EXISTS (SELECT id FROM tailoring_package WHERE tailoring_package.tailoring_type_id = ?)`,
                            [id, id, id], async function (err: QueryError, data: any) {
                                if (err || data.affectedRows == 0) {
                                    ...
                                    res.status(200).json({ success: false, msg: 'Error,This Item Already Used, Cannot Delete', newdata: [] });
                                    res.end();
                                    con.end();
                                    return;
                                } else {
                        ...
                        con.query(`DELETE FROM tailoring_sizes WHERE id = ? LIMIT 1`,
                            [id], async function (err: QueryError, data: any) {
                                if (err || data.affectedRows == 0) {
                                    res.setHeader('Content-Type', 'application/json');
                                    res.status(200).json({ success: false, msg: 'Error,This Item Already Used, Cannot Delete', newdata: [] });
                                    res.end();
