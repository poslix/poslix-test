                    ...
                    if (subType == 'getAll') {
                        ...
                        FROM transactions
                            INNER JOIN contacts ON contacts.id = transactions.contact_id
                                WHERE transactions.type = 'purchase' and transactions.location_id = ? ORDER BY id DESC;`, [shopId], function (err: QueryError, data: any) {
                            if (err) {
                                console.log('errr');
                                console.log(err);
                        ...
                        con.query(`SELECT
                                ...
                                GROUP BY tl.id, tl.product_id, tl.variation_id;`, [purchaseId], function (err: QueryError, data: any) {
                            if (err) {
                                console.log('errr');
                                console.log(err);
                                res.setHeader('Content-Type', 'application/json');
                        ...
                        const { id } = req.body;
                        con.query(`SELECT id as 'value',name as 'label',type,id as product_id,name,sku,sell_price,cost_price,qty_over_sold AS 'qty_sold'
                            FROM products 
                                WHERE type NOT IN ('package','tailoring_package') AND products.is_service = 0 and location_id = ? ORDER BY id DESC;`, [shopId], function (err: QueryError, products: any) {

                            con.query(`SELECT id AS 'value', CONCAT(first_name,' ',last_name) AS 'label'
                                    FROM contacts
                                        WHERE  contacts.location_id = ? AND contacts.type = 'supplier' ORDER BY id DESC;`, [shopId], function (err: QueryError, suppliers: any) {

                                con.query(`SELECT id AS 'value', name AS 'label'
                                        FROM expenses
                                            WHERE  location_id = ? ORDER BY id DESC;SELECT id AS 'value',concat(country,' (',code,')') AS 'label',exchange_rate,symbol,code  FROM currencies`, [shopId], async function (err: QueryError, exp_currency: any) {
                                    ...
                                    await con.promise()
                                        ...
                                        product_variations.cost AS 'variation_cost'
                                            FROM product_variations
                                                WHERE product_variations.location_id = ?`, [shopId])
                                        .then((rows: any, fields: any) => {
                                            if (rows[0].length > 0)
                                                allVariations = rows[0]
                                        })
                                        .catch((err: QueryError) => { })

                                    ///get purchase for edit
                                    var purchase: any = [], selected_expnses: any = [], selected_payment: any = [], selected_lines: any = []
                                    if (id > 0) {

                                        //get invoice detials
                                        await con.promise()
                                            .query(`SELECT * FROM transactions WHERE location_id = ? and id = ? LIMIT 1`, [shopId, id])
                                            .then((rows: any, fields: any) => {
                                                if (rows[0].length > 0)
                                                    purchase = rows[0]
                                            })
                                        ...
                                        await con.promise()
                                            ...
                                            expenses_values.id,expenses_values.expense_id,expenses_values.value,expenses_values.entered_value AS enterd_value,expenses.name AS 'label',expenses_values.currency_id,expenses_values.currency_rate
                                                ...
                                                WHERE expenses.location_id = ? and expenses_values.transaction_id = ?`, [shopId, id])
                                            .then((rows: any, fields: any) => {
                                                if (rows[0].length > 0)
                                                    selected_expnses = rows[0]
                                            })
                                            .catch((err: QueryError) => {
                                                console.log(err);
                                            })
                                        //get payment
                                        await con.promise()
                                            .query(`SELECT * FROM transaction_payments WHERE transaction_id = ?  ORDER BY id ASC LIMIT 1 `, [id])
                                            .then((rows: any, fields: any) => {
                                                if (rows[0].length > 0)
                                                    selected_payment = rows[0]
                                            })
                                            .catch((err: QueryError) => {
                                                console.log(err);
                                            })
                                    ...
                                    res.status(200).json({
                                        ...
                                        newdata: { products, allVariations, suppliers, expenses: exp_currency[0], currencies: exp_currency[1], purchase, selected_expnses, selected_payment, selected_lines }
                                    });
                                    res.end();
                                    con.end();
                                    return;
                        ...
                        var con = doConnect()
                        con.query(`SELECT * FROM transaction_payments WHERE transaction_id = ?`, [purchaseId], function (err: QueryError, data: any) {
                            if (err) {
                                console.log('errr');
                                console.log(err);
                                res.setHeader('Content-Type', 'application/json');
                        ...
                        await con.promise()
                            .query(`SELECT invoice_details FROM business_locations WHERE business_locations.id = ? AND owner_id = ?`,
                        ...
                        WHERE transactions.location_id = ? AND transactions.type = 'sell' ORDER BY id DESC LIMIT 500 ;`,
                            ...
                            function (err: QueryError, data: any) {
                                if (err) {
                                    ...
                                    res.setHeader("Content-Type", "application/json");
                                    res.status(200).json({ success: true, msg: "its done", newdata: { data, invoiceDetails } });
                                    res.end();
                                    con.end();
                                    return;
                                }
                        ...
                        var con = doConnect();
                        const { id } = req.body;
                        con.query(`SELECT tl.id,tl.product_id,tl.tax_amount,tl.variation_id,CONCAT_WS(' ',s.name,pv.name) AS 'name',ts.notes,s.type,s.is_service,tl.stock_id,tl.qty,tl.cost,tl.price,ts.contact_id,ts.total_price,CONCAT_WS(' ',cont.first_name,cont.last_name) AS 'contact_name',s.is_fabric,tl.tailoring_txt,tl.note,s.is_tailoring,tl.tailoring_custom,COALESCE(tl.tailoring_link_num,0) as tailoring_link_num
                        FROM transactions_lines tl
                        ...
                        await con.promise()
                            .query(`SELECT id, status, created_at FROM cash_registers WHERE location_id = ? ORDER BY id DESC LIMIT 1`, [shopId])
                            .then((rows: any, fields: any) => {
                                if (rows[0].length > 0) {
                        ...
                        console.log('frooom ', fda2, " to ", ed2);

                        con.query(`SELECT SUM(ts.total_price) AS price,tp.payment_type,max(ts.id)
