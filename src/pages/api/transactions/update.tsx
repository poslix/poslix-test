        ...
        if (locationPermission(repo.data.locs, shopId) != -1) {
          ...
          if (subType == 'editPurchase') {
            ...
            lines.map(async (itm: any, idx: number) => {
              if (itm.isNew)
                ...
                await con
                  .promise()
                  .query(
                    'UPDATE transactions_lines SET price = ?,cost = ?,cost_type = ?,qty = ? WHERE id = ?',
            ...
            if (sqlValueLines.length > 0) {
              await con
                .promise()
                .query(sqlInsertLines, [sqlValueLines])
                .then((rows: any, fields: any) => {
                  console.log('lines ', rows);
                })
                .catch();
            ...
            await con
              .promise()
              .query('UPDATE transaction_payments SET payment_type = ?,amount = ? WHERE id = ?', [
                totalOrder.paymentType,
                totalOrder.paid_amount,
                totalOrder.payment_id,
              ])
              .then((rows: any, fields: any) => {})
              .catch();
