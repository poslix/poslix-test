    ...
    try {
        ...
        if (req.body.type == 'editProduct') {
            ...
            con.query(`UPDATE products SET 
                ...
                WHERE id=?`,
                [data.productName, data.productName2, 59, img, data.tax_id, 'inclusive', data.sku, 'EAN13', data.unit, data.brand, data.cat, data.alertQuantity, 59, 0, 0, 0, data.purchasePrice, data.sellPrice, data.id],
        ...
        if (req.body.fetch == 'editCustomerInfo') {
            ...
            con.query(`UPDATE contacts SET first_name = ?,last_name = ?,mobile = ?,city = ?,state = ?,country = ?,address_line_1 = ?,address_line_2 = ?,zip_code = ?,shipping_address = ? WHERE id=?`,
                [req.body.data.firstName, req.body.data.lastName, req.body.data.mobile, req.body.data.city, req.body.data.state, req.body.data.country, req.body.data.addr1, req.body.data.addr2, req.body.data.zipCode, req.body.data.shipAddr, req.body.data.id], function (err: QueryError, prods: RowDataPacket[]) {
                    if (err) {
                        res.setHeader('Content-Type', 'application/json');
                        res.status(200).json({ success: false, msg: 'error' + err, newdata: [] });
                        res.end();
                        ...
                        res.setHeader('Content-Type', 'application/json');
                        const newOne = { id: req.body.data.id, firstName: req.body.data.firstName, mobile: req.body.data.mobile }
                        res.status(200).json({ success: true, msg: 'customer info edited', newdata: newOne });
                        res.end();
                        con.end();
                        return;
        ...
        if (req.body.type == 'locationBusinessSettings') {
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    var con = doConnect();
                    con.query(`UPDATE business_locations SET currency_id= ?,decimal_places = ? WHERE id = ? and owner_id = ? limit 1`,
                        [locationSetting.currency_id, locationSetting.decimal_places, data.locationId, repo.data.id],
                        function (err: QueryError, prods: RowDataPacket[]) {
                            if (err) {
                                res.setHeader('Content-Type', 'application/json');
                                res.status(200).json({ success: false, msg: 'error' + err, newdata: [] });
