            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == 'getLastOrders') {
                        const getAll = `SELECT transactions.id,CONCAT_WS(' ',contacts.first_name,contacts.last_name) AS 'name',contacts.id AS 'customer_id',transactions.location_id,transactions.total_price,transactions.status,transactions.created_at,transactions.created_by 
                        FROM transactions 
