            ...
            if (repo.status === true) {
                ...
                if (locationPermission(repo.data.locs, shopId) != -1) {
                    ...
                    if (subType == 'initDash') {
                        ...
                        let factors_lsit: any = [], productsOverSold: any = [], salse_list: any = [];
                        //get top box sales
                        let _sales: any = [];
                        ...
                        console.log('_sales ', profit_sales);

                        //get top box expnese
                        let _expense: any = [];
                        const profit_expense: any = [];
                        await con.promise()
                            .query(`SELECT sum(amount) as total FROM expenses_list 
                            ...
                            SELECT sum(amount) as total FROM expenses_list 
                            WHERE location_id = ? AND YEAR(created_at) = YEAR(CURRENT_DATE());`, [shopId, shopId, shopId, shopId])
                            .then((rows: any, fields: any) => {
                                _expense = rows[0]
                            })
                            .catch((err: QueryError) => { })
                        ...
                        console.log('_expense ', profit_expense);
                        ...
                        await con.promise()
                            ...
                            SELECT sum(total_price) as total FROM transactions
                            WHERE location_id = ?  AND type = 'purchase' AND YEAR(created_at) = YEAR(CURRENT_DATE());`, [shopId, shopId, shopId, shopId])
                            .then((rows: any, fields: any) => {
                                _purchase = rows[0]
                            })
                            .catch((err: QueryError) => { })
                        ...
                        await con.promise()
                            ...
                            SELECT count(id) AS total FROM contacts
                            WHERE location_id = ? 
                            AND YEAR(created_at) = YEAR(CURRENT_DATE());`, [shopId, shopId, shopId, shopId])
                            .then((rows: any, fields: any) => {
                                _contacts = rows[0]
                            })
                            .catch((err: QueryError) => { })
                        ...
                        console.log('_contacts ', count_contacts);
                        ...
                        let _months: any = [], months_name = [month1, month2, month3];
                        await con.promise()
                            .query(`SELECT sum(total_price) AS total FROM transactions
                        ...
                        SELECT sum(total_price) AS total FROM transactions
                        WHERE location_id = ? AND month(created_at) = month(DATE_SUB(CURRENT_DATE(), INTERVAL ? MONTH))
                        AND year(created_at) = year(DATE_SUB(CURRENT_DATE(), INTERVAL ? MONTH));`, [shopId, 0, 0, shopId + 800, 1, 1, shopId, 2, 2])
                            .then((rows: any, fields: any) => {
                                _months = rows[0]
                            })
                            .catch((err: QueryError) => {
                        ...
                        await con.promise()
                            .query(`SELECT transactions.id,transactions.total_price,contacts.name AS 'contact_name',users.first_name AS 'created_by',transactions.created_at
                                FROM transactions
                        ...
                        let top_down_pros: any = [], upPro: any = [], downPro: any = [];
                        await con.promise()
                            .query(`SELECT 
                                    ...
                                    t.created_at >= DATE_SUB(NOW(), INTERVAL 1 year)
                                    GROUP by tl.product_id,tl.variation_id
                                    ORDER BY total_qty_sold LIMIT 7;`, [shopId, shopId])
                            .then((rows: any, fields: any) => {
