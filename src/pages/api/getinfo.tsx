...
export default async function handler(
    ...
    try {
            ...
            FROM products
                ...
                INNER JOIN brands ON brands.id = products.brand_id
                    where length(image) > 3 And products.business_id = ? 
                            LIMIT 500`, [bId], function (err: QueryError, prods: RowDataPacket[]) {
                if (err) throw err;
                //get catgories
                con.query(`SELECT categories.id,categories.name 
                            FROM categories
                                WHERE categories.business_id = ?`, [bId], function (err: QueryError, cats: RowDataPacket[]) {
                    if (err) throw err;
                    //get brands
                    con.query(`SELECT brands.id,brands.name 
                                FROM brands
                                    WHERE brands.business_id = ?`, [bId], function (err: QueryError, brands: RowDataPacket[]) {
                        if (err) throw err;
                        //get contacts
                        con.query(`SELECT contacts.id as 'value',CONCAT_WS(' | ',CONCAT_WS(' ',contacts.first_name,contacts.last_name),contacts.mobile) AS 'label',contacts.mobile
                                    FROM contacts
                                        WHERE contacts.business_id = ? AND contacts.type = 'customer' ORDER BY RAND()`, [bId], function (err: QueryError, custo: RowDataPacket[]) {
                            if (err) throw err;
                            //get group default Tax if has
                            con.query(`SELECT id,name,amount,type AS 'amountType',tax_type as 'taxType',is_primary as 'isPrimary'
                                FROM tax_rates 
                                    WHERE id IN ((SELECT tax_id FROM tax_group 
                                            WHERE parent_id = (SELECT id FROM tax_rates where business_id = ? AND is_primary = 1 AND tax_type ='group')));`, [bId], function (err: QueryError, taxes: RowDataPacket[]) {
                                if (err) throw err;
                                //
                                con.query(`SELECT tax_rates.id,tax_rates.name,tax_rates.amount,tax_rates.tax_type as 'taxType', tax_rates.is_primary as'isPrimary',tax_rates.type AS 'amountType',p.parent_id as 'parentId' FROM tax_rates
                                    INNER JOIN (SELECT tax_id,parent_id,business_id FROM tax_group where business_id = ?) AS p ON p.tax_id=tax_rates.id
                                        WHERE p.business_id = ?`, [bId, bId], function (err: QueryError, taxGroup: ITax[]) {
                                    if (err) throw err;
                                    var readTaxGroup = groupCalculation(taxGroup)

                                    con.query(`SELECT id,status,closing_amount FROM cash_registers WHERE user_id = 1 ORDER BY id desc LIMIT 1`, [bId, bId], function (err: QueryError, cash: ITax[]) {
                                        if (err) throw err;

                                        res.setHeader('Content-Type', 'application/json');
                                        res.json({ success: true, msg: 'done!', data: { prods: prods, cats: cats, brands: brands, customers: custo, taxes: taxes, tax_group: readTaxGroup, cash: cash } });
                                        res.end();
                                        con.end()
                                        return;
                                    });
        ...
        if (req.body.fetch == 'getCustomerInfo') {
            var con = doConnect();
            con.query(`SELECT id,first_name,last_name,city,state,country,address_line_1 AS 'addr1',address_line_2 AS 'addr2' ,mobile,country,zip_code,shipping_address
            FROM contacts
                WHERE id = ?`, [req.body.recordId], function (err: QueryError, prods: RowDataPacket[]) {
                if (err) {
                    res.setHeader('Content-Type', 'application/json');
        ...
        if (req.body.fetch == 'getLastOrders') {
            ...
            con.query((req.body.recordId == -1 ? getAll : getOne), [req.body.recordId], function (err: QueryError, prods: RowDataPacket[]) {
                if (err) {
                    res.setHeader('Content-Type', 'application/json');
                    res.status(200).json({ success: false, msg: 'error', newdata: [] });
                    res.end();
                ...
                transactions.id,CONCAT_WS(' ',contacts.first_name,contacts.last_name) AS 'name',contacts.id AS 'customer_id',transactions.business_id,transactions.final_total,transactions.status,transactions.transaction_date,transactions.expired_product_return_total_price,transactions.created_by 
                    FROM transactions 
                        INNER JOIN contacts ON transactions.contact_id = contacts.id
                            WHERE Length(contacts.first_name)>0
                                ORDER BY transaction_date desc LIMIT 10`,
                [req.body.recordId], function (err: QueryError, prods: RowDataPacket[]) {
                    if (err) {
                        res.setHeader('Content-Type', 'application/json');
                        res.status(200).json({ success: false, msg: 'error', newdata: [] });
                        res.end();
        ...
        if (req.body.fetch == 'getBusiness') {
            ...
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    var con = doConnect();
                    con.query(`SELECT business_locations.name AS 'loc_name',business_locations.state,business_locations.id AS 'loc_id',business.id AS 'business_id',business.name AS 'business_name'
                    ...
                    FROM business_locations
                        ...
                        inner JOIN business_types ON business_types.id = business.type_id
                            WHERE business.owner_id = ?`, [repo.data.id], function (err: QueryError, data: any) {
                        if (err) {
                            console.log(err);
                            res.setHeader('Content-Type', 'application/json');
                    ...
                    console.log('loginnnnnnnnnnnnnn first');
        ...
        if (req.body.fetch == 'getBusinessSettings') {
            ...
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    con.query(`SELECT business.name,business.email_settings as 'email',business_types.name as 'business_type'
                                FROM business 
                                    INNER JOIN business_types ON business_types.id = business.type_id
                                        WHERE business.owner_id = ? AND business.id = ?`, [repo.data.id, businessId], function (err: QueryError, genSettings: any) {

                        con.query(`SELECT id,name,country,state,city,currency_id,decimal_places
                            FROM business_locations
                                WHERE owner_id = ? AND business_id = ?`, [repo.data.id, businessId], function (err: QueryError, locations: any) {

                            con.query(`SELECT users.id as 'value',users.first_name as 'label',user_stuff.stuff,user_stuff.stuff_ids,user_stuff.location_id as 'locationId'
                                FROM users
                                    INNER JOIN user_stuff ON user_stuff.user_id = users.id
                                        WHERE users.owner_id = ?`, [repo.data.id], function (err: QueryError, users: any) {

                                con.query(`SELECT users.id as 'value',users.first_name as 'label'
                                    FROM users WHERE users.owner_id = ?`, [repo.data.id], function (err: QueryError, allusers: any) {

                                    con.query(`SELECT id AS 'value',concat(country,' (',code,')') AS 'label' FROM currencies`, function (err: QueryError, currencies: any) {

                                        con.query(`SELECT id AS 'value',name AS 'label' FROM stuffs where owner_id = ?`, [repo.data.id], function (err: QueryError, roles: any) {
                                            if (err) {
                                                console.log(err);
                                                res.setHeader('Content-Type', 'application/json');
                                                ...
                                                res.setHeader('Content-Type', 'application/json');
                                                res.status(200).json({ success: true, msg: 'its done', newdata: { general: genSettings, locations, users, allusers, currencies, roles } });
                                                res.end();
                                                con.end();
                                            }
        ...
        if (req.body.fetch == 'getUsersMyBusiness') {
            ...
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    con.query(`SELECT first_name AS 'name',username,email,contact_number as 'mobile',password FROM users
                    WHERE users.owner_id = ?`, [repo.data.id], function (err: QueryError, myusers: any) {
                        if (err) {
                            console.log(err);
                            res.setHeader('Content-Type', 'application/json');
                            res.status(200).json({ success: false, msg: 'error1' + err });
                            ...
                            res.setHeader('Content-Type', 'application/json');
                            res.status(200).json({ success: true, msg: 'its done', newdata: { myusers: myusers } });
                            res.end();
                            con.end();
                        }
        ...
        if (req.body.fetch == 'getStuffsMyBusiness') {
            ...
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    con.query(`SELECT * FROM stuffs
                        WHERE owner_id = ?`, [repo.data.id], function (err: QueryError, myStuffs: any) {
                        if (err) {
                            console.log(err);
                            res.setHeader('Content-Type', 'application/json');
                            res.status(200).json({ success: false, msg: 'error1' + err });
                            ...
                            res.setHeader('Content-Type', 'application/json');
                            res.status(200).json({ success: true, msg: 'its done', newdata: { myStuffs: myStuffs } });
                            res.end();
                            con.end();
                        }
        ...
        verifayTokens(req, async (repo: ITokenVerfy) => {
            if (repo.status === true) {
                const { data } = req.body;
                console.log(data.shopId, repo.data.locs);
                if (locationPermission(repo.data.locs, data.shopId) != -1) {
                    //init add/edit pages
                    if (req.body.fetch == 'initProductData') {
                        var con = doConnect();
                        con.query(`SELECT id as 'value',name as 'label' FROM barcodes WHERE barcodes.business_id = 59`, function (err: QueryError, reps_bars: any) {

                            con.query(`SELECT id as 'value',short_name as 'label' FROM units WHERE units.business_id = 59;`, function (err: QueryError, reps_unit: any) {

                                con.query(`SELECT id as 'value',name as 'label' FROM brands WHERE brands.business_id = 59;`, function (err: QueryError, reps_brands: any) {

                                    con.query(`SELECT id as 'value',name as 'label' FROM categories WHERE categories.business_id = 59 ORDER BY id desc`, function (err: QueryError, reps_cats: any) {

                                        con.query(`SELECT id as 'value',name as 'label' FROM business_locations WHERE business_locations.business_id = 59 ORDER BY id desc`, function (err: QueryError, reps_locs: any) {

                                            con.query(`SELECT id AS 'value', name AS 'label',amount
                                                        FROM tax_rates
                                                            WHERE  business_id = ? AND tax_type = 'group' ORDER BY id DESC;`, [59], function (err: QueryError, tax: any) {
                                                if (req.body.id != 0) {
                                                    con.query(`SELECT * FROM products WHERE products.id = ? ORDER BY id desc`, [req.body.id], function (err: QueryError, proDetails: any) {
                                                        res.setHeader('Content-Type', 'application/json');
                                                        res.status(200).json({ success: true, msg: 'its done1', newdata: { barcodes: reps_bars, units: reps_unit, brands: reps_brands, cats: reps_cats, locations: reps_locs, pro: proDetails, taxes: tax } });
                                                        res.end();
                                                        con.end();
                                                        return;
                                                    });
                                                    ...
                                                    res.setHeader('Content-Type', 'application/json');
                                                    res.status(200).json({ success: true, msg: 'its done2', newdata: { barcodes: reps_bars, units: reps_unit, brands: reps_brands, cats: reps_cats, locations: reps_locs, taxes: tax, pro: [] } });
                                                    res.end();
                                                    con.end();
                                                    return;
                                                }
                    ...
                    if (req.body.fetch == 'initPurchases') {
                        var con = doConnect();
                        con.query(`SELECT id as 'value',name as 'label',id,name,sku,sell_price,purchase_price as 'cost_price'
                        FROM products
                            WHERE business_id = ? ORDER BY id DESC;`, [59], function (err: QueryError, pros: any) {

                            con.query(`SELECT id AS 'value', NAME AS 'label'
                            FROM business_locations
                                WHERE  business_locations.business_id= ?  ORDER BY id DESC;`, [59], function (err: QueryError, locations: any) {

                                con.query(`SELECT id AS 'value', CONCAT(' ',NAME,supplier_business_name) AS 'label'
                                FROM contacts
                                    WHERE  contacts.business_id = ? AND contacts.type = 'supplier' ORDER BY id DESC;`, [59], function (err: QueryError, supps: any) {

                                    con.query(`SELECT id AS 'value', name AS 'label',Round(amount/100,2) as 'amount'
                                    FROM tax_rates
                                        WHERE  business_id = ? AND tax_type = 'group' ORDER BY id DESC;`, [59], function (err: QueryError, taxs: any) {

                                        con.query(`SELECT id AS 'value', name AS 'label'
                                    FROM expenses
                                        WHERE  business_id = 59 ORDER BY id DESC;`, [59], function (err: QueryError, expenses: any) {

                                            con.query(`SELECT tax_rates.id,tax_rates.name,tax_rates.amount,tax_rates.tax_type as 'taxType', tax_rates.is_primary as'isPrimary',tax_rates.type AS 'amountType',p.parent_id as 'parentId' FROM tax_rates
                                            INNER JOIN (SELECT tax_id,parent_id,business_id FROM tax_group where business_id = ?) AS p ON p.tax_id=tax_rates.id
                                                WHERE p.business_id = ?`, [59, 59], function (err: QueryError, rwoGroupData: ITax[]) {
                                                ...
                                                res.setHeader('Content-Type', 'application/json');
                                                res.status(200).json({
                                                    success: true, msg: 'its done',
                                                    newdata: { products: pros, locations: locations, suppliers: supps, taxs: taxs, expenses: expenses, tax_group: readTaxGroup }
                                                });
                                                res.end();
                                                con.end();
                                                return;
                    ...
                    if (req.body.fetch == 'initCateBrand') {
                        ...
                        await con.promise().query(`SELECT id AS 'value', concat(NAME,' (',Round(amount,2),')') AS 'label',amount
                        FROM tax_rates
                            WHERE  business_id = ? AND tax_type = 'group' ORDER BY id DESC;`, [59], function (err: QueryError, taxs: any) {
                    ...
                    if (req.body.fetch == "getSales") {
                        var con = doConnect();
                        con.query(`SELECT transactions.id ,
                            ...
                            function (err: QueryError, data: any) {
                                if (err) {
                                    ...
                                    res
                                        .status(200)
                                        .json({ success: false, msg: "error1" + err, newdata: [] });
                                    res.end();
                                    con.end();
                                    return;
                                } else {
                                    ...
                                    res
                                        .status(200)
                                        .json({ success: true, msg: "its done", newdata: data });
                                    res.end();
                                    con.end();
                    ...
                    if (req.body.fetch == "initSalesData") {
                        ...
                        con.query(`SELECT products.id,variations.id AS 'varId',products.sku,variations.default_sell_price AS 'price',products.tax AS 'product_tax' ,products.name AS 'product', categories.tax_id AS 'category_tax',
                                brands.tax_id AS 'brand_tax',
                                ...
                                FROM products
                                    ...
                                    And products.business_id = ?`, [data.shopId],
                                        ...
                                        con.query(
                                            ...
                                            function (err: QueryError, rwoGroupData: ITax[]) {
                                                ...
                                                con.query(`SELECT id,name,amount,type AS 'amountType',tax_type as 'taxType',is_primary as 'isPrimary'
                                                    ...
                                                    [59], function (err: QueryError, taxes: RowDataPacket[]) {
                                                        res.status(200).json({
                                                            success: true,
                                                            msg: "its done1",
                                                            newdata: {
        ...
        if (req.body.fetch == 'getProducts') {
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    const { businessId } = req.body;
                    console.log('in ', businessId, repo.data.id);

                    var con = doConnect();
                    con.query(`SELECT products.id,products.name,products.business_id,products.image,products.sku,products.barcode_type,products.alert_quantity,products.cost_price,products.sell_price,
                    ...
                    categories.name AS 'category'
                        FROM products
                            ...
                            INNER JOIN business_locations ON business_locations.id = products.location_id
                                WHERE products.location_id = ? AND business_locations.owner_id = ? ORDER BY id DESC;`, [businessId, repo.data.oid! > 0 ? repo.data.oid! : repo.data.id], function (err: QueryError, data: any) {
                        if (err) {
                            console.log(err);
                            res.setHeader('Content-Type', 'application/json');
                            res.status(200).json({ success: false, msg: 'error1' + err, newdata: [] });
        ...
        if (req.body.fetch == 'getPurchases') {
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    const { businessId } = req.body;
                    console.log(businessId, repo.data.locs);
                    if (locationPermission(repo.data.locs, businessId) != -1) {
                        var con = doConnect();
                        con.query(`SELECT transactions.created_at,transactions.id,CONCAT(' ',contacts.first_name,contacts.last_name) AS 'supplier',transactions.total_price,transactions.ref_no,STATUS,payment_status
                        FROM transactions
                            INNER JOIN contacts ON contacts.id = transactions.contact_id
                                WHERE transactions.type = 'purchase' and transactions.location_id = ? ORDER BY id DESC;`, [businessId], function (err: QueryError, data: any) {
                            if (err) {
                                console.log('errr');
                                console.log(err);
                                res.setHeader('Content-Type', 'application/json');
        ...
        if (req.body.fetch == 'getExpense') {
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    if (locationPermission(repo.data.locs, businessId) != -1) {
                        var con = doConnect();
                        con.query(`SELECT id,name FROM expenses
                            WHERE  business_id = ? ORDER BY id DESC;`, [businessId], function (err: QueryError, data: any) {
                            if (err) {
                                console.log(err);
                                res.setHeader('Content-Type', 'application/json');
                                res.status(200).json({ success: false, msg: 'error1' + err, newdata: [] });
        ...
        if (req.body.fetch == 'getTaxs') {
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    if (locationPermission(repo.data.locs, businessId) != -1) {
                        var con = doConnect();
                        con.query(`SELECT id,name,amount,type as 'amountType',IF(is_primary = 1,TRUE,FALSE) AS 'isPrimary',tax_type as 'taxType',false as 'isNew' FROM tax_rates
                            WHERE  business_id = ?;`, [businessId], function (err: QueryError, data: any) {
                            res.setHeader('Content-Type', 'application/json');
                            res.status(200).json({ success: true, msg: 'its done', newdata: data });
                            res.end();
                            con.end();
        ...
        if (req.body.fetch == 'getCatsAndBrands') {
            ...
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    if (locationPermission(repo.data.locs, businessId) != -1) {
                        var con = doConnect();
                        con.query(`SELECT id,name
                                    FROM categories
                                            WHERE location_id = ? ORDER BY id desc;`, [businessId], function (err: QueryError, cates: any) {
                            con.query(`SELECT id,name
                                FROM brands
                                    WHERE  location_id = ? ORDER BY id desc;`, [businessId], function (err: QueryError, brands: any) {

                                res.setHeader('Content-Type', 'application/json');
                                res.status(200).json({ success: true, msg: 'its done', newdata: { cates: cates, brands: brands } });
                                res.end();
                                con.end();
        ...
        if (req.body.fetch == 'getOneProduct') {
            ...
            con.query(`SELECT 
                ...
                FROM products
                    ...
                    INNER JOIN brands ON brands.id = products.brand_id
                    INNER JOIN categories ON categories.id = products.category_id
                WHERE products.business_id = 59 ORDER BY id DESC;`, [59], function (err: QueryError, data: any) {
                if (err) {
                    console.log(err);
                    res.setHeader('Content-Type', 'application/json');
                    res.status(200).json({ success: false, msg: 'error1' + err, newdata: [] });
        ...
        if (req.body.fetch == 'getGroupItems') {
            ...
            verifayTokens(req, (repo: ITokenVerfy) => {
                if (repo.status === true) {
                    ...
                    if (locationPermission(repo.data.locs, businessId) != -1) {
                        const { id } = req.body;
                        con.query(`SELECT tax_rates.id as 'tax_id',tax_rates.name,tax_rates.amount,tax_rates.type,tax_rates.tax_type,tax_group.id
                            FROM tax_rates
                                INNER JOIN tax_group ON tax_group.tax_id = tax_rates.id
                                    WHERE tax_rates.business_id = ? AND tax_group.parent_id = ? ORDER BY tax_rates.id DESC;`, [businessId, id], function (err: QueryError, data: any) {
                            if (err) {
                                console.log(err);
                                res.setHeader('Content-Type', 'application/json');
                                res.status(200).json({ success: false, msg: 'error1' + err, newdata: [] });
