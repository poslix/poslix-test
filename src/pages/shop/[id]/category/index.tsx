...
const Category = ({ shopId, rules }: any) => {
    ...
    async function initDataPage() {
        const { success, newdata } = await apiFetchCtr({ fetch: 'categery_brand', subType: 'getCatsAndBrands', shopId })
        console.log(newdata);
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <AlertDialog alertShow={show} alertFun={(e: boolean) => setShow(e)} id={selectId} expenses={cats} >
                    Are you Sure You Want Delete This Item ?
                </AlertDialog>
                <div className="row">
                    {rules.hasInsert && <div className='mb-4'><button className='btn btn-primary p-3' onClick={() => {
                        console.log(key);
                        ...
                        if (key === 'Categories')
                            ...
                            Toastify('error', "Error On Add New")
                    }}><FontAwesomeIcon icon={faPlus} /> Add New {key} </button></div>}
                    <Tabs
                                                    ...
                                                    return (
                                                        <tr key={i} className='td-style-table'>
                                                            ...
                                                            <td><ButtonGroup className="mb-2 m-buttons-style">
                                                                {rules.hasEdit && <Button onClick={() => router.push({ pathname: '/shop/' + shopId + '/category/edit/' + ex.id, query: { type: 'category' } })}><FontAwesomeIcon icon={faPenToSquare} /></Button>}
                                        ...
                                        <tbody>
                                            {
                                                brands.map((ex: any, i: number) => {
                                                    return (
                                                        <tr key={i} className='td-style-table'>
                                                            <th scope="row">{ex.id}</th>
                                                            <td style={{ color: ex.tax_name != null || ex.never_tax == 1 ? '#eb8181' : '' }}>{ex.name}</td>
                                                            <td>{ex.tax_name}</td>
                                                            <td><ButtonGroup className="mb-2 m-buttons-style">
                                                                {rules.hasEdit && <Button onClick={() => router.push({ pathname: '/shop/' + shopId + '/category/edit/' + ex.id, query: { type: 'brand' } })}><FontAwesomeIcon icon={faPenToSquare} />
                                                                </Button>}
                                                                {rules.hasDelete && <Button onClick={() => { setSelectId(ex.id); setShow(true) }}><FontAwesomeIcon icon={faTrash} /></Button>}
                                                            </ButtonGroup></td>
                                                        </tr>)
