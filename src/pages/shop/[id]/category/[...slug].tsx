...
const Product: NextPage = (probs: any) => {
    ...
    const [formObj, setFormObj] = useState({ id: 0, name: '', des: '', tax: null })
    ...
    async function initDataPage(id = '0', type = '') {
        if (id != '0')
            setIsEdit(true);
        var result = await apiFetchCtr({ fetch: 'categery_brand', subType: 'initCateBrand', shopId, id, type })
        ...
        setTaxGroup([{ value: null, label: 'Default Tax' }, { value: -1, label: 'Never Tax' }, ...newdata.taxes])
        console.log(newdata);
    ...
    async function insertHandle() {
        var result = await apiInsertCtr({ type: 'categery_brand', subType: 'insert_' + pageType, shopId, data: { frmobj: formObjRef.current } })
        const { success } = result;
        if (success) {
            Toastify('success', 'successfuly added')
            setTimeout(() => { router.push('/shop/' + shopId + '/category') }, 1000);
        } else {
            alert("Has Error ,try Again")
        }
    }
    async function editHandle() {
        var result = await apiUpdateCtr({ type: 'categery_brand', subType: 'edit_' + pageType, shopId, data: { frmobj: formObjRef.current } })
        const { success } = result;
        if (success) {
            Toastify('success', 'successfuly added')
            setTimeout(() => { router.push('/shop/' + shopId + '/category') }, 1000);
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <ToastContainer />
                <div className="row"><div className='mb-4'><Link className='btn btn-primary p-3' href={'/shop/' + shopId + '/category'}>Back To List</Link></div></div>
                <Card className="mb-4">
                    <Card.Header className="p-3 bg-white">
                        <h5>{isEdit ? 'Edit ' + pageType : 'Add New ' + pageType}</h5>
                    </Card.Header>
                    <Card.Body>
                        {!loading ? <form className='form-style'>

                            <div className="form-group2">
                                <label>Name: <span className='text-danger'>*</span></label>
                                <input type="text" className="form-control" placeholder=" Enter Name" value={formObj.name}
                            ...
                            <div className="row">
                                <div className="form-group">
                                    ...
                                    <input type="text" className="form-control" placeholder="description"
                                        value={formObj.des}
                                        onChange={(e) => { setFormObj({ ...formObj, des: e.target.value }) }} />
                                </div>
                                ...
                                <div className="form-group">
                                    ...
                                    <Select
                                        styles={colourStyles}
                                        options={taxGroup}
                                        value={taxGroup.filter((f: any) => { return f.value == formObj.tax })}
                                        onChange={(itm) => { setFormObj({ ...formObj, tax: itm!.value }) }}
                                    />
                            ...
                            <button type="button" className="btn m-btn btn-primary p-2 "
                                onClick={(e) => {
                                    ...
                                    if (errors.length == 0) {
                                        ...
                                        alert('Enter Requires Field')
                                }} >{isEdit ? 'Edit' : 'Save'}</button>
                        </form> : <div className='d-flex justify-content-around' ><Spinner animation="grow" /></div>}
                    </Card.Body>
                </Card>
            </AdminLayout >
        </>
...
export async function getServerSideProps(context: any) {
    ...
    if (_addOrEdit == undefined || (_addOrEdit != 'add' && _addOrEdit != 'edit'))
        _isOk = false;
    if (_pageType == undefined || (_pageType != 'category' && _pageType != 'brand'))
        _isOk = false;
    if (!_isOk || context.query.id == undefined)
        _isOk = false;
    if (!_isOk)
    ...
    //status ok
    return {
        props: { shopId, pageType: _pageType, editId: _addOrEdit == 'edit' ? _EditId : 0 },
    };
}