...
import { faTrash, faPenToSquare, faPlus, faTag, faArrowRight, faArrowDown, faPencil, faArrowLeft, faEye } from '@fortawesome/free-solid-svg-icons'
...
import { DataGrid, GridColDef, GridRowParams, GridToolbarColumnsButton, GridToolbarContainer, GridToolbarExport, GridToolbarQuickFilter } from '@mui/x-data-grid';
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <ToastContainer />
                <AlertDialog alertShow={show} alertFun={(e: boolean) => setShow(e)} id={selectId} type="deleteProduct" products={orders} >
                ...
                {!isLoading ?
                    <div className='orders-tailoring-container'>
                        ...
                        {isShowDetails ? <div className="order-details-box">
                            <div className="order-details-body">
                                <button onClick={() => setIsShowDetails(false)} type='button' className='btn mt-4 p-3 mb-2 btn-primary'><FontAwesomeIcon icon={faArrowLeft} /> Back</button>
                                <div className='contact-info'>
                                    <div><img src='/images/dashboard/user.jpg' /></div>
                                    <div><span>Name:</span><br />{selectedOrders.tname}</div>
                                    <div><span>Phone:</span><br />{selectedOrders.contact_mobile}</div>
                                    <div><span>Tailoring Type:</span><br />{selectedOrders.type_name}</div>
                                    <div><span>Order ID:</span><br />{selectedOrders.transaction_id}</div>
                                    <div><span>Date:</span><br />{selectedOrders.created_at.split('T')[0]}</div>
                                    <div><span>Status:</span><br />{selectedOrders.status} <Button onClick={() => handleShow(selectedIndex)}><FontAwesomeIcon icon={faPencil} /></Button></div>
                                </div>
                                {(changeStatus.st && changeStatus.i == selectedIndex) && <div className='status-mini-modal-cont'>
                                    <p>Change To</p>
                                    <div className='status-mini-modal'>
                                        <div className="mini-item"><button onClick={() => handleClick(selectedOrders.id, 'pending', selectedIndex)} type='button' className='btn btn-primary'>Pending</button> </div>
                                        <div className="mini-item"><button onClick={() => handleClick(selectedOrders.id, 'processing', selectedIndex)} type='button' className='btn btn-primary'>Processing</button> </div>
                                        <div className="mini-item"><button onClick={() => handleClick(selectedOrders.id, 'complete', selectedIndex)} type='button' className='btn btn-primary'>Complete</button> </div>
                                    </div>
                                </div>}

                                <div className="order-details-items">
                                    {selectedSizes.map((size, i) => {
                                        return (<div key={i + 1} className="order-details-item" >
                                            <span>{(size.name == "_frm_name" ? "name" : size.name)}</span> <br /> {size.value}</div>)
                                    })}
                                </div>
                                ...
                                <div className="tailoring-notes">
                                    ...
                                    {tailoringExtras.map((te, i: number) => {
                                        return (<p key={i}><span>{extras.find((ex: any) => ex.id == te.id)?.name}</span> {te.value}</p>)
                                    })}
                                </div>
