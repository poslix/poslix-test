...
const AddSale: NextPage = (props: any) => {
    ...
    const discountType = [{ value: "fixed", label: "Fixed" }, { value: "percentage", label: "Percentage" }];
    ...
    async function initDataPage(id = "0") {
        console.log('get init data aaaaa');
    ...
    useEffect(() => {
        var _items = 0, _prices_perItem = 0, _total_price = 0;
        selectProducts.map((p: IsaleProductItem) => {
            ...
            if (p.def_tax)
                _total_price += finalCalculation(defTaxGroup, _prices_perItem);
            else if (p.product_tax > 0)
                _total_price += finalCalculation(
                    readyTaxGroup[p.product_tax],
                    _prices_perItem
                );
            else if (p.brand_tax > 0)
                _total_price += finalCalculation(
                    readyTaxGroup[p.brand_tax],
                    _prices_perItem
                );
            else if (p.cat_tax > 0)
                _total_price += finalCalculation(
                    readyTaxGroup[p.cat_tax],
                    _prices_perItem
                );
    ...
    async function insertSale() {
        ...
        const { success, msg } = await apiInsert({ type: "transactionSale", data: { shopId, items: holdObj, details }, });
        if (success) {
