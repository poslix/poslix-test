...
export default function SalesList(props: any) {
            ...
            renderCell: ({ row }: Partial<GridRowParams>) => (
                <>
                    <ButtonGroup className="mb-2 m-buttons-style">
                        ...
                        <Button onClick={() => { onRowsSelectionHandler(row) }}><FontAwesomeIcon icon={faEye} /></Button>
                    </ButtonGroup>
                </>
            ),
        },
    ...
    async function initDataPage() {
        const { success, newdata } = await apiFetchCtr({ fetch: "transactions", subType: "getSales", shopId });
        if (success) {
    ...
    return (
        <AdminLayout shopId={shopId}>
            ...
            <button className="mb-3 btn btn-primary p-3" onClick={() => router.push("/shop/" + shopId + "/sales/add")}><FontAwesomeIcon icon={faPlus} /> Add New Sales</button>
            <div className="page-content-style card">
                ...
                <DataGrid
                    ...
                    rows={sales}
                    columns={columns}
                    initialState={{ columns: { columnVisibilityModel: { mobile: false } } }}
                    pageSize={10}
