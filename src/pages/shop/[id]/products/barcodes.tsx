...
const Product: NextPage = (probs: any) => {
    ...
    const [options, setOptions] = useState<{ name: boolean, category: boolean, price: boolean, businessName: boolean }>({ name: false, category: false, price: false, businessName: false })
    const [selectedProducts, setSelectedProducts] = useState<{ id: number, product_id: number, variation_id: number, sku: string, name: string, price: number, quantity: number, category: string }[]>([]);
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: "quantity", headerName: "qty", editable: true, type: "number"
        },
        {
            field: "action",
            headerName: "Action ",
    ...
    async function initDataPage() {
        const { success, data } = await apiFetchCtr({ fetch: 'products', subType: 'initBarcodePage', shopId })
        if (!success) {
    ...
    const addPackageProducts = (e: any) => {
        ...
        if (!found) setSelectedProducts([...selectedProducts, { id: e.product_id, product_id: e.product_id, variation_id: 0, name: e.name, quantity: 1, price: e.price, category: e.category, sku: e.sku }])
        ...
        if (jobType.req == 4) {
            ...
            allVariations.map((varItm: any, index: number) => {
                if (varItm.variation_id == jobType.val) {
                    ...
                    if (!found)
                        setSelectedProducts([...selectedProducts, { id: +Number(varItm.product_id) + Math.floor(Math.random() * 1200), product_id: varItm.product_id, variation_id: varItm.variation_id, name: selectedProductForVariation.product_name + ' ' + varItm.name, quantity: 1, price: varItm.variation_price, sku: varItm.sku, category: 'no set' }])
                    else Toastify('error', "already exists in list")
                }
            })
        }
                    ...
                    for (let i = 0; i < sp.quantity; i++) {
                        items.push(
                            <div key={`${sp.sku}-${i}`} style={{ height: '120px' }}>
                                {options.businessName && <h6 style={{ textAlign: 'center', fontSize: '20px' }}>{locationSettings.label}</h6>}
                                {options.name && <h6 style={{ textAlign: 'center', fontSize: '20px' }}>{sp.name}</h6>}
                                {options.price && <h6 style={{ textAlign: 'center', fontSize: '20px' }}>{Number(sp.price).toFixed(locationSettings.currency_decimal_places)} {locationSettings.currency_code}</h6>}
                                {options.category && <h6 style={{ textAlign: 'center', fontSize: '20px' }}>{sp.category}</h6>}
                                <div style={{ textAlign: 'center' }}><BarcodeGenerator sku={sp.sku} /></div>
                            </div>
                        );
                    }
                    return items;
            ...
            <AdminLayout shopId={shopId}>
                ...
                {!isLoading ? <>
                    <div className="page-content-style card">
                        ...
                        <h5>Step1: <span style={{ color: '#cdc8c8' }}>Selecet Products</span></h5>
                        <Select
                            styles={colourStyles}
                            options={products}
                        ...
                        />
                        <h5>Step2: <span style={{ color: '#cdc8c8' }}>Options</span></h5>
                        <div className='invoice-settings-body' style={{ maxWidth: '500px' }}>
                            <div className='invoice-settings-item'>
                                <div>Business Name</div>
                                <div><Form.Check type="switch" className="custom-switch" onChange={(e) => { setOptions({ ...options, businessName: e.target.checked }) }} /></div>
                            </div>
                            <div className='invoice-settings-item'>
                                <div>Product Name</div>
                                <div><Form.Check type="switch" className="custom-switch" onChange={(e) => { setOptions({ ...options, name: e.target.checked }) }} /></div>
                            </div>
                            <div className='invoice-settings-item'>
                                <div>Price</div>
                                <div><Form.Check type="switch" className="custom-switch" onChange={(e) => { setOptions({ ...options, price: e.target.checked }) }} /></div>
                            </div>
                            <div className='invoice-settings-item'>
                                <div>Category</div>
                                <div><Form.Check type="switch" className="custom-switch" onChange={(e) => { setOptions({ ...options, category: e.target.checked }) }} /></div>
                            </div>
                        </div>
                        <button type="button" className="btn btn-primary p-2" onClick={handlePrint} style={{ width: '100%', maxWidth: '500px', marginTop: '10px' }}>Show</button>
