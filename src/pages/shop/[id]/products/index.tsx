...
import { faTrash, faPenToSquare, faPlus, faTag, faBarcode } from '@fortawesome/free-solid-svg-icons'
...
const Product: NextPage = (probs: any) => {
    ...
    const columns: GridColDef[] = [
        ...
        { field: "sell_price", headerName: "sell", flex: 1, renderCell: (params) => Number(params.value).toFixed(locationSettings.currency_decimal_places) },
        { field: "category", headerName: "Category", flex: 1 },
        {
            field: "qty", headerName: "Qty", flex: 0.5,
            renderCell: ({ row }: Partial<GridRowParams>) => (
                <>
                    <div className={row.qty > 0 && row.type != 'package' ? 'clickable-qty' : ''}>{row.type != 'package' ? row.qty : '---'}<span className='qty-over'>[{Number(row.qty_over_sold).toFixed(1)}]</span></div>
                </>
            )
        },
        {
            ...
            renderCell: ({ row }: Partial<GridRowParams>) => (
                <>
                    <ButtonGroup className="mb-2 m-buttons-style">
                        ...
                        <Button onClick={() => { router.push('/shop/' + shopId + '/products/barcodes/') }}><FontAwesomeIcon icon={faBarcode} /></Button>
                    </ButtonGroup>
                </>
            ),
        },
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                ...
                {/* start */}
                {!isLoading && rules.hasInsert && <div className='mb-2'><button className='btn btn-primary p-3' onClick={() => router.push('/shop/' + shopId + '/products/add')}><FontAwesomeIcon icon={faPlus} /> Add New Product </button></div>}
                {!isLoading ? <>
