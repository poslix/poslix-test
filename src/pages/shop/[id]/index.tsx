...
import { GridColDef, GridToolbarContainer, GridToolbarExport, GridToolbarColumnsButton, GridToolbarFilterButton } from "@mui/x-data-grid";
...
const Home = (probs: any) => {
  ...
  const [topProdcutsDown, setTopProdcutsDown] = useState({ labels: [], values: [] })
  ...
  async function initData() {
    ...
    const { success, data } = await apiFetchCtr({ fetch: 'dashboard', subType: 'initDash', shopId })
    if (!success) {
    ...
    if (data.upPro != null && data.upPro.length > 0)
      setTopProdcuts({ labels: data.upPro.map((item: any) => item.name), values: data.upPro.map((item: any) => item.total_qty_sold) });
    else
      setTopProdcuts({ labels: [0, 0, 0, 0, 0, 0, 0], values: [0, 0, 0, 0, 0, 0, 0] });
    if (data.downPro != null && data.downPro.length > 0)
      setTopProdcutsDown({ labels: data.downPro.map((item: any) => item.name), values: data.downPro.map((item: any) => item.total_qty_sold) });
    else
      setTopProdcutsDown({ labels: [0, 0, 0, 0, 0, 0, 0], values: [0, 0, 0, 0, 0, 0, 0] });
  }
  useEffect(() => {
    initData()
  ...
  const btnHandleTimeFrame = (index: number, p: number) => {
    ...
    if (p == 1)
      setTxtP1({ name: getTxtTimeFrame(index), index: index })
    else if (p == 2)
      setTxtP2({ name: getTxtTimeFrame(index), index: index })
    else if (p == 3)
      setTxtP3({ name: getTxtTimeFrame(index), index: index })
    else if (p == 4)
      setTxtP4({ name: getTxtTimeFrame(index), index: index })
  }
  useEffect(() => {
    ...
    <AdminLayout shopId={shopId}>
      <div className="row loc-dash-top" style={{ background: '#f6f8fa' }}>
        <div className='loc-dash-top-items'>
          ...
          <div className='inner-loc-dash-top-items dash-top-items-details'>
            ...
            <h3>{Number(box1Price[txtP1.index - 1]).toFixed(locationSettings.currency_decimal_places)}<span>{locationSettings.currency_code}</span></h3>
          </div>
          <div className='inner-loc-dash-top-items arrows-details'>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP1.index + 1, 1)}><FontAwesomeIcon icon={faArrowUp} /></div>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP1.index - 1, 1)}><FontAwesomeIcon icon={faArrowDown} /></div>
          </div>
        </div>
        ...
        <div className='loc-dash-top-items'>
          ...
          <div className='inner-loc-dash-top-items dash-top-items-details'>
            ...
            <h3>{Number(box3Price[txtP3.index - 1]).toFixed(locationSettings.currency_decimal_places)}<span>{locationSettings.currency_code}</span></h3>
          </div>
          <div className='inner-loc-dash-top-items arrows-details'>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP3.index + 1, 3)}><FontAwesomeIcon icon={faArrowUp} /></div>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP3.index - 1, 3)}><FontAwesomeIcon icon={faArrowDown} /></div>
          </div>
        </div>
        ...
        <div className='loc-dash-top-items'>
          ...
          <div className='inner-loc-dash-top-items dash-top-items-details'>
            ...
            <h3>{Number(box2Price[txtP2.index - 1]).toFixed(locationSettings.currency_decimal_places)}<span>{locationSettings.currency_code}</span></h3>
          </div>
          <div className='inner-loc-dash-top-items arrows-details'>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP2.index + 1, 2)}><FontAwesomeIcon icon={faArrowUp} /></div>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP2.index - 1, 2)}><FontAwesomeIcon icon={faArrowDown} /></div>
          </div>
        </div>
        ...
        <div className='loc-dash-top-items'>
          ...
          <div className='inner-loc-dash-top-items dash-top-items-details'>
            ...
            <h3>{Number(box4Price[txtP4.index - 1])}<span></span></h3>
          </div>
          <div className='inner-loc-dash-top-items arrows-details'>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP4.index + 1, 4)}><FontAwesomeIcon icon={faArrowUp} /></div>
            <div className='arrow-updown' onClick={() => btnHandleTimeFrame(txtP4.index - 1, 4)}><FontAwesomeIcon icon={faArrowDown} /></div>
          </div>
        </div>
      </div>
      <div className="row loc-dash-top-under">
        ...
        <div className='loc-dash-small-chart'>
          <h4>Top 7 Products <Switch checked={upDown} onChange={handleUpDown} /><span>{upDown ? 'Down' : 'Up'}</span></h4>
          <div>
            <Bar
              data={data_last_low}
              width={400}
                ...
                facrtors.map((itm, i: number) => {
                  return (
                    <div key={i} className='me-tr-table'>
                      ...
                      <div className="m-fileds">{Number(itm.total_price).toFixed(locationSettings.currency_decimal_places)} {locationSettings.currency_code}</div>
