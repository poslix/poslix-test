...
import { faTrash, faFloppyDisk, faPenToSquare, faPaperclip, faPlus, faArrowAltCircleLeft } from '@fortawesome/free-solid-svg-icons'
...
const Product: NextPage = (props: any) => {
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: "attach", headerName: "Attach File", flex: 1,
            renderCell: ({ row }: Partial<GridRowParams>) => (
        ...
        },
        {
            ...
            renderCell: ({ row }: Partial<GridRowParams>) => (
                <>
                    <ButtonGroup className="mb-2 m-buttons-style">
                        {rules.hasEdit && <Button onClick={() => {
                            setSelectId(row.id)
                            setIsAddExpense(true);
                        }}><FontAwesomeIcon icon={faPenToSquare} /></Button>}
                        {rules.hasDelete && <Button onClick={() => { setSelectId(row.id); setShow(true) }}><FontAwesomeIcon icon={faTrash} /></Button>}
                    </ButtonGroup>
                </>
            ),
        },
    ...
    async function initDataPage() {
        setIsLoading(true)
        const { success, newdata } = await apiFetchCtr({ fetch: 'expenses', subType: 'getCateAndList', shopId })
        console.log(success);
    ...
    async function addUpdateExpense() {
        const { success, data } = await apiInsertCtr({ type: 'expenses', subType: 'insetUpdateExpenes', data: cate, shopId })
        if (!success) {
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <ToastContainer />
                <AlertDialog alertShow={show} id={selectId} alertFun={handleDeleteFuc} shopId={shopId} type="expenses" subType="deleteExpense" >
                        ...
                        <Tab eventKey="list" title="List">
                            <Card>
                                ...
                                <Card.Body style={{ minHeight: '650px ' }}>
                                    {!isLoading && rules.hasInsert && <div className='mb-2'><button className='btn btn-primary p-3' onClick={() => handlebtnAdd()}>
                                        {isAddExpense ? <><FontAwesomeIcon icon={faArrowAltCircleLeft} /> back </> : <><FontAwesomeIcon icon={faPlus} /> Add Expense </>}</button></div>}
                                    {!isAddExpense ? <>
                                        <DataGrid
                                        ...
                                        : <AddNewExpeness selectId={selectId} setExpensesList={setExpensesList} rows={expensesList} setIsAddExpense={setIsAddExpense} shopId={shopId} cats={cate} />}
                                </Card.Body>
                            </Card>
                        ...
                        <Tab eventKey="category" title="Category">
                            <Card>
                                                    ...
                                                    return (
                                                        <tr key={i} style={{ background: ex.isNew ? '#c6e9e6' : '' }}>
                                                            ...
                                                            <td><ButtonGroup className="mb-2 m-buttons-style">
                                                                {rules.hasDelete && <Button onClick={() => { setSelectId(ex.id); setShow(true) }}><FontAwesomeIcon icon={faTrash} /></Button>}
                                                            </ButtonGroup></td>
                                                        </tr>)
                                                })
                                            }
                                ...
                                {rules.hasInsert && <div className='m-3'>
                                    <button className='btn m-btn btn-primary p-3' onClick={() => addUpdateExpense()}><FontAwesomeIcon icon={faFloppyDisk} /> save</button>
                                </div>}
                            </Card>
                        </Tab>
                    </Tabs>
