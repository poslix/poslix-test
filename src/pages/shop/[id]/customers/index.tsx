...
const Product: NextPage = (props: any) => {
    ...
    const [customers, setCustomers] = useState<{ id: number, name: string, mobile: string }[]>([])
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <ToastContainer />
                <AlertDialog alertShow={show} alertFun={handleDeleteFuc} shopId={shopId} id={selectId} type="customers" subType="deleteCustomer">
                    Are you Sure You Want Delete This Item ?
                </AlertDialog>
                {/* start */}
                {!isLoading && rules.hasInsert && <div className='mb-2'><button className='btn btn-primary p-3' onClick={() => router.push('/shop/' + shopId + '/customers/add')}><FontAwesomeIcon icon={faPlus} /> Add New Customer </button></div>}
                {!isLoading ? <>
