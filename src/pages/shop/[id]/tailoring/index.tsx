    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <ToastContainer />
                <AlertDialog alertShow={show} alertFun={handleDelete} shopId={shopId} id={selectId} subType="delTailoring" type="tailoring" products={tailoring} >
                    ...
                    <div className="col-md-12">
                        <Tabs
                            ...
                            <Tab eventKey="types" title="Types">
                                {rules.hasInsert && <div className='mb-4'><Link className='btn btn-primary p-3' href={'/shop/' + shopId + '/tailoring/add'}><FontAwesomeIcon icon={faPlus} /> Add New Tailoring Type </Link></div>}
                                <Card>
                                    ...
                                    <Card.Body className='table-responsive text-nowrap'>
                                        {!isLoading ? <Table className="table table-hover" responsive>
                                            ...
                                            <tbody>
                                                {
                                                    tailoring.length > 0 ? tailoring.map((pro: any, index: number) => {
                                                        return (
                                                            <tr key={index}>
                                                                <th scope="row">{pro.id}</th>
                                                                <td>{pro.type_name} / {pro.multiple_value} <span className='under-text'>[{pro.name}]</span></td>
                                                                <td><ButtonGroup className="mb-2 m-buttons-style">
                                                                    {rules.hasEdit && <Button onClick={() => {
                                                                        router.push('/shop/' + shopId + '/tailoring/edit/' + pro.id)
                                                                    }}><FontAwesomeIcon icon={faPenToSquare} /></Button>}
                                                                    {rules.hasDelete && <Button onClick={() => { setSelectId(pro.id); setShow(true) }}><FontAwesomeIcon icon={faTrash} /></Button>}
                                                                </ButtonGroup>
                                                                </td>
                                                            </tr>)
                                                    }) : <tr><th colSpan={9} className='no-products' >No tailoring items</th></tr>
                                                }
                                            </tbody>
                                        </Table>
                            ...
                            <Tab eventKey="extras" title="Extras">
                                {rules.hasInsert && <div className='mb-4'><Link className='btn btn-primary p-3' href={'/shop/' + shopId + '/tailoring/extra/add'}><FontAwesomeIcon icon={faPlus} /> Add New Extra </Link></div>}
                                <Card>
                                    <Card.Header className="p-3 bg-white">
                                        <h5>Extras List</h5>
                                    </Card.Header>
                                    <Card.Body className='table-responsive text-nowrap'>
                                        {!isLoading ? <Table className="table table-hover" responsive>
                                            ...
                                            <tbody>
                                                {
                                                    extras.length > 0 ? extras.map((pro: any, index: number) => {
                                                        return (
                                                            <tr key={index}>
                                                                <th scope="row">{pro.id}</th>
                                                                <td>{pro.name}</td>
                                                                ...
                                                                <td><ButtonGroup className="mb-2 m-buttons-style">
                                                                    {rules.hasEdit && <Button onClick={() => {
                                                                        router.push('/shop/' + shopId + '/tailoring/extra/edit/' + pro.id)
                                                                    }}><FontAwesomeIcon icon={faPenToSquare} /></Button>}
                                                                    {rules.hasDelete && <Button onClick={() => { setSelectId(pro.id); setShow(true) }}><FontAwesomeIcon icon={faTrash} /></Button>}
                                                                </ButtonGroup>
                                                                </td>
