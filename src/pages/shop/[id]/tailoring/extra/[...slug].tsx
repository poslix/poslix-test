...
const ExtraManage: NextPage = (props: any) => {
    ...
    const [formObj, setFormObj] = useState<any>({
        id: 0, name: '', isRequired: false, items: [{ name: '' }]
    })
    const [errorForm, setErrorForm] = useState({ name: false, items: false, multiple: false, primary: false })
    ...
    async function initDataPage(id = "0") {
        ...
        setIsEdit(true)
        setIsLoading(true)
        const { success, data } = await apiFetchCtr({ fetch: 'tailoring', subType: 'getInitExtra', shopId, id })
        if (success) {
    ...
    async function addExtras() {
        const { success } = await apiInsertCtr({ type: 'tailoring', subType: 'addExtras', data: formObj, shopId })
        if (!success) {
    ...
    async function UpdateTailoring() {
        const { success } = await apiUpdateCtr({ type: 'tailoring', subType: 'updateExtra', data: formObj, shopId, id: editId })
        if (!success) {
            Toastify('error', "Has Error ,try Again")
            return;
        }
                    ...
                    <div className="col-md-12">
                        ...
                        <Card className='mt-3' style={{ borderRadius: '10px' }}>
                            ...
                            <Card.Body className='table-responsive text-nowrap'>
                                {!isLoading ? <Table className="table table-hover">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td><input type="text" name="tax-name" className="form-control p-2" placeholder="Enter A Title" value={formObj.name} onChange={(e) => { setFormObj({ ...formObj, name: e.target.value }) }} /></td>
                                            <td>
                                                <Table className="table table-hover">
                                                    <thead className="thead-dark"><tr></tr></thead>
                                                    <tbody>
                                                        {
                                                            formObj.items.map((ex: any, j: number) => {
                                                                return (
                                                                    <tr key={j * 82}>
                                                                        <td><input type="text" name="tax-name" className="form-control p-2" placeholder="Enter Option Name" value={ex.name} onChange={(e) => { handleInputChange(e, j) }} /></td>
                                                                        <td><ButtonGroup className="mb-2 m-buttons-style">
                                                                            {rules.hasDelete && <Button onClick={() => { handleDeleteMan(j) }}><FontAwesomeIcon icon={faTrash} /></Button>}
                                                                        </ButtonGroup></td>
                                                                    </tr>)
                                                            })
                                                        }
                                                        <tr>
                                                            <td><ButtonGroup className="mb-2 m-buttons-style">
                                                                <Button onClick={() => { handleAddItem() }}><FontAwesomeIcon icon={faPlus} /> Add filed</Button>
                                                            </ButtonGroup></td>
                                                        </tr>
                                                    </tbody>
                                                </Table>
                                            ...
                                            <td><Form.Check type="switch" id="custom-switch" disabled={!rules.hasInsert} className="custom-switch" checked={formObj.isRequired} onChange={(e) => setFormObj({ ...formObj, isRequired: !formObj.isRequired })} /></td>
                                        </tr>
                                    </tbody>
                        ...
                        {rules.hasInsert && <div className='m-3'>
                            <button className='btn m-btn btn-primary p-3' onClick={() => {
                                ...
                                if (formObj.items.length == 0 || formObj.items[0].name.toString().trim().length == 0) errors.push('error')
                                setErrorForm({
                                    ...errorForm,
                                    name: formObj.name.length == 0,
                                    items: formObj.items.length == 0 || formObj.items[0].name.toString().trim().length == 0,
                                })
                                if (errors.length == 0) {
                                    if (isEdit)
                                        UpdateTailoring()
