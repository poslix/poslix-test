...
const AddTailoring: NextPage = (props: any) => {
    ...
    const [formObj, setFormObj] = useState<any>({
        id: 0, name: '', multiple: 0,
        sizes: [{ name: '', isPrimary: false, isNew: true }],
        extras: [{ name: 'addOns', isRequired: false, items: [{ name: 'item1', isNew: true }, { name: 'item2', isNew: true }] }]
    })
    const [errorForm, setErrorForm] = useState({ name: false, sizes: false, multiple: false, primary: false })
    ...
    async function initDataPage(id = "0") {
        setIsLoading(true)
        const { success, data } = await apiFetchCtr({ fetch: 'tailoring', subType: 'getInitPage', shopId, id })
        if (success) {
            console.log('dd ', data);
            ...
            if (id != "0") {
                ...
                console.log('d', data.extras.filter((ex: any) => extraIds.includes(ex.value + ",")));
    ...
    async function addTailoring() {
        const { success } = await apiInsertCtr({ type: 'tailoring', subType: 'tailoringadd', data2: { data: formObj, selectedExtras }, shopId })
        if (!success) {
    ...
    async function UpdateTailoring() {
        const { success } = await apiUpdateCtr({ type: 'tailoring', subType: 'tailoringedit', data2: { data: formObj, selectedExtras }, shopId, id: editId })
        if (!success) {
            Toastify('error', "Has Error ,try Again")
            return;
        }
    ...
    const handleAddItem = () => {
        ...
        _rows.push({ name: '', items: [{ name: 'item1', isNew: true }, { name: 'item2', isNew: true }], isNew: true })
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <ToastContainer />
                <AlertDialog alertShow={show} alertFun={handleDelete} shopId={shopId} id={selectId} subType="deleteSizeItem" type="tailoring" products={formObj.sizes} >
                    Are you Sure You Want Delete This Item ?
                </AlertDialog>
                <div className="row"><div className='mb-3'><Link className='btn btn-primary p-3' href={'/shop/' + shopId + '/tailoring'}>Back To List</Link></div></div>
                <div className="row">
                    <div className="col-md-12">
                        <Card>
                            ...
                            <Card.Body className='form-style'>
                                ...
                                <div className="form-group2">
                                    <label>multiple: <span className='text-danger'>*</span></label>
                                    <input type="number" className="form-control" placeholder="multiple"
                                        value={formObj.multiple}
                                        onChange={(e) => { setFormObj({ ...formObj, multiple: +e.target.value }) }} />
                                    {errorForm.multiple && <p className='p-1 h6 text-danger '>Enter multiple</p>}
                                </div>
                            </Card.Body>
                        </Card>
                        {formObj.name.length > 0 && <Card className='mt-3' style={{ borderRadius: '10px' }}>
                            <Card.Header className="p-3 bg-white">
                                <h5>Add Sizes For {formObj.name}</h5>
                            </Card.Header>
                            <Card.Body className='table-responsive text-nowrap'>
                                {errorForm.primary && <p className='p-1 h6 text-danger '>One Item Must Be Primary</p>}
                                    ...
                                    <tbody>
                                        {
                                            formObj.sizes.map((ex: any, i: number) => {
                                                return (
                                                    <tr key={i}>
                                                        <th scope="row">{i + 1}</th>
                                                        <td><input type="text" disabled={!rules.hasInsert} name="tax-name" className="form-control p-2" placeholder="Enter New Tax Name" value={ex.name} onChange={(e) => { handleInputChange(e, i) }} /></td>
                                                        <td><Form.Check type="switch" id="custom-switch" disabled={!rules.hasInsert} className="custom-switch" checked={ex.isPrimary} onChange={(e) => { handlePrimarySwitchChange(e, i) }} /></td>
                                                        <td><ButtonGroup className="mb-2 m-buttons-style">
                                                            {rules.hasDelete && <Button onClick={() => { handleDeleteMan(i) }}><FontAwesomeIcon icon={faTrash} /></Button>}
                                                        </ButtonGroup></td>
                                                    </tr>)
                                            })
                                        }
                        ...
                        {rules.hasInsert && <div className='m-3'>
                            <button className='btn m-btn btn-primary p-3' onClick={() => {
                                errors = [];
                                if (formObj.name.length == 0) errors.push('error')
                                if (formObj.multiple.length == 0) errors.push('error')
                                if (formObj.sizes.length == 0 || formObj.sizes[0].name.toString().trim().length == 0) errors.push('error')
                                let _has = true;
                                ...
                                setErrorForm({
                                    ...
                                    sizes: formObj.sizes.length == 0 || formObj.sizes[0].name.toString().trim().length == 0,
