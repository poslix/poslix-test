...
import { faTrash, faPenToSquare, faPlus, faDollarSign, faCheckDouble, faListCheck } from '@fortawesome/free-solid-svg-icons'
...
const Purchases: NextPage = (probs: any) => {
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: "status", headerName: "Stock Status", flex: 0.5,
        ...
        },
        {
            ...
            renderCell: ({ row }: Partial<GridRowParams>) => (
                getStatusStyle(row.payment_status)
            ),
        },

        { field: "total_price", headerName: "Total Price", flex: 1, renderCell: (params) => Number(params.value).toFixed(locationSettings.currency_decimal_places) },
        {
            field: "action",
            headerName: "Action ",
            sortable: false,
            ...
            renderCell: ({ row }: Partial<GridRowParams>) => (
                <>
                    <ButtonGroup className="mb-2 m-buttons-style">
                        <Button disabled={row.status == "draft"} onClick={() => {
                            console.log('row ', row);
                            setPurchaseId(row.id)
                            setIsShowQtyManager(!isShowQtyManager)
                        }}><FontAwesomeIcon icon={faListCheck} /></Button>
                        <Button disabled={row.status == "draft"} onClick={() => {
                            setPurchaseId(row.id)
                            setIsShowPayments(!isShowPayments)
                        }}><FontAwesomeIcon icon={faDollarSign} /></Button>
                        <Button onClick={() => router.push('/shop/' + shopId + '/purchases/edit/' + row.id)}><FontAwesomeIcon icon={faPenToSquare} /></Button>
    ...
    async function initDataPage() {
        const { success, newdata } = await apiFetchCtr({ fetch: 'transactions', subType: 'getAll', shopId })
        if (success) {
    ...
    function getStatusStyle(status: string) {
        switch (status) {
            case 'paid':
                return (<div className=''><span className='purchase-satus-style'>{status}</span></div>)
            case 'received':
                return (<div className=''><span className='purchase-satus-style'>{status}</span></div>)
            default:
                return status;
    ...
    return (
        <>
            <AdminLayout shopId={shopId}>
                <ToastContainer />
                {isShowQtyManager && <PurchasesQtyCheckList selectedIndex={selectedIndex} purchases={purchases} locationSettings={locationSettings} shopId={shopId} purchaseId={purchaseId} setIsShowQtyManager={setIsShowQtyManager} />}
                {isShowPayments && <PurchasePaymentsList selectedIndex={selectedIndex} purchases={purchases} shopId={shopId} purchaseId={purchaseId} setIsShowPayments={setIsShowPayments} />}
                {!isShowQtyManager && !isShowPayments && <div className="row">
                    <div className="col-md-12">
                        {!isloading && <div className='mb-4'><button className='btn m-btn btn-primary p-3' onClick={() => { router.push('/shop/' + shopId + '/purchases/add') }}><FontAwesomeIcon icon={faPlus} /> New Purchase </button></div>}
                        {!isloading ? <>
