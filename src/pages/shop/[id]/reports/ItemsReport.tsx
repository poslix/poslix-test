...
export default function ItemsReport(props: any) {
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: "qty", headerName: "qty", maxWidth: 72,
            renderCell: ({ row }: Partial<GridRowParams>) => (
                Number(row.qty).toFixed(2)
            )
        },
        {
            field: "price", headerName: "Sub Total", maxWidth: 80,
            renderCell: ({ row }: Partial<GridRowParams>) => (
                Number(row.price).toFixed(locationSettings.currency_decimal_places)
            )
        },
        {
            field: "tax_amount", headerName: "Tax", maxWidth: 72, disableColumnMenu: true,
            renderCell: ({ row }: Partial<GridRowParams>) => (
                Number(row.tax_amount).toFixed(locationSettings.currency_decimal_places)
            )
        },
        {
            field: "total_price", headerName: "Total", maxWidth: 72,
            renderCell: ({ row }: Partial<GridRowParams>) => (
                Number(row.total_price).toFixed(locationSettings.currency_decimal_places)
            )
        },
        {
            field: "cost", headerName: "Cost", maxWidth: 72, disableColumnMenu: true,
            renderCell: ({ row }: Partial<GridRowParams>) => (
    ...
    async function initDataPage() {
        ...
        const { success, data } = await apiFetchCtr({ fetch: "reports", subType: "getItemsReport", shopId });
        if (success) {
            setsales(data.sales);
        ...
        <AdminLayout shopId={shopId}>
            ...
            <div className="page-content-style card">
                ...
                <div className="deatils_box">
                    ...
                    <div><span>Cost: </span>{Number(details.cost).toFixed(3)} {locationSettings.currency_code}</div>
                </div>
                <DataGrid
                ...
                <DialogContent className="poslix-modal-content">
                    <div className="poslix-modal">
                        <div className="top-section-details">
                            ...
                            <div><Button onClick={() => { handlePrint() }}>Print Recipt</Button> <Button>Print Invoice</Button></div>
                        </div>
                        {lines && !isLoadItems ?
                            <div className="row">
