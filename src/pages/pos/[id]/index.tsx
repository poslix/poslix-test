...
const Home: NextPage = (probs: any) => {
  ...
  const { setCats, setBrands, setProducts, setCustomers, setTaxes, setTaxGroups, setVariations, setPackageItems } = useContext(ProductContext);
  const { setLocationSettings, setTailoringSizes, setInvoicDetails, setTailoringExtras } = useContext(UserContext);
  ...
  useEffect(() => {
    ...
    else if (jobType.req == 102)
      initData();
  }, [jobType])
  async function initData() {
    const { success, data } = await apiFetchCtr({ fetch: 'pos', subType: 'getPosInit', shopId })
    if (!success) {
  ...
  async function openRegister() {
    const { success } = await apiInsertCtr({ type: 'customer', subType: 'opens', cashHand, shopId })
    if (!success) {
    ...
    <>
      ...
      <div id="layout-wrapper">
        ...
        <div className="main-content">
          ...
          {!isLoading ? isOpenRegister ? <div className="pos-flex">
            <NavMenu shopId={shopId} />
            <OrdersComponent shopId={shopId} />
            <ItemList />
          </div> :
            <div className="pos-flex">
              <div className="open-register">
                ...
                <button className='btn btn-primary p-3'
                  onClick={() => openRegister()}><FontAwesomeIcon icon={faCashRegister} /> Open Register </button>
              </div>
            </div> : <div className='pos-loading'>
            <div>
