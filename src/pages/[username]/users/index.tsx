...
const Locations = ({ username }: any) => {
    ...
    async function initDataPage() {
        const { success, newdata } = await apiFetchCtr({ fetch: 'owner', subType: 'getUsersMyBusiness' })
        if (!success) {
            alert("error")
            return
        }
    ...
    return (
        <>
            <OwnerAdminLayout>
                ...
                }}><FontAwesomeIcon icon={!isAddUser ? faPlus : faArrowAltCircleLeft} /> {!isAddUser ? 'Add User' : 'Back'}</button>
                {isAddUser && <AddNewUser setIsAddUser={setIsAddUser} users={users} index={selectedId} />}
                {!isAddUser && <div className="row">
