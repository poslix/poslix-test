...
const Locations = ({ username, businessId, pageType }: any) => {
    ...
    return (
        <>
            {pageType == 'settings' && <BusinessSettings username={username} businessId={businessId} ></BusinessSettings>}
...
export function getServerSideProps(context: any) {
    let okUsername = true, okBusinessId = true, okSlug = true
    ...
    var pageType = "";
    if (context.query.slug.length > 0)
        pageType = context.query.slug;

    if (pageType != "add" && pageType != "settings")
        okSlug = false
